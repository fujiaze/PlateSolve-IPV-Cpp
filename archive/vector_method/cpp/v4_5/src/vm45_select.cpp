// ============================================================================
// vm45_select.cpp - V4.5 StarSelector 模块 (Phase 0) (复用 V4.4 算法)
//
// 职责: 图像侧选星 + Gaia 侧不对称密度匹配查询
// 复用 V4.4 vm44_select.cpp 的算法, 仅做 namespace/前缀替换 + output.s0 赋值:
//   - namespace v44 -> v45
//   - 常量前缀 VM44_ -> VM45_
//   - 主入口 vm44_select -> vm45_select
//   - 类型 VM44SolveParams -> VM45SolveParams
//   - include vm44_internal.h -> vm45_internal.h
//   - 新增: output.s0 = s0 (V4.5 StarSelection 新增 s0 字段, 供 Phase A 使用)
//
// 核心算法 (与 V4.4 一致, 从 V4.2 ss_core.cpp 迁移):
...
static constexpr double VM45_ARCSEC_PER_UM_PER_MM = 206.265;
static constexpr double VM45_DEGTORAD = VM45_PI / 180.0;
static constexpr double VM45_RADTODEG = 180.0 / VM45_PI;
// 角秒/弧度 = 180×3600/π
static constexpr double VM45_ASEC_PER_RAD = 206264.80624709636;
...
static bool load_dlls(Logger* logger) {
    ...
    if (!g_dll.aio_dll) {
        if (logger) logger->error("vm45_select: 无法加载 astro_image_io.dll");
    ...
    if (!g_dll.aio_read || !g_dll.aio_get_pixel_data ||
        ...
        if (logger) logger->error("vm45_select: astro_image_io 函数符号解析失败");
        g_dll.load_failed = true;
        return false;
    }
    ...
    if (!g_dll.sdet_dll) {
        if (logger) logger->error("vm45_select: 无法加载 star_detector.dll");
        g_dll.load_failed = true;
        return false;
    }
    ...
    if (!g_dll.sdet_detect_ex || !g_dll.sdet_free_ex) {
        if (logger) logger->error("vm45_select: star_detector 函数符号解析失败");
        g_dll.load_failed = true;
        return false;
    }
    ...
    if (!g_dll.gaia_dll) {
        if (logger) logger->error("vm45_select: 无法加载 gaia_client.dll");
        g_dll.load_failed = true;
        return false;
    }
    ...
    if (!g_dll.gaia_cone_search) {
        if (logger) logger->error("vm45_select: gaia_client_cone_search_for_solver 符号解析失败");
        g_dll.load_failed = true;
        return false;
    }

    g_dll.loaded = true;
    if (logger) logger->info("vm45_select: 依赖 DLL 全部加载成功");
    return true;
}
...
// 非 Windows 平台 stub
static bool load_dlls(Logger* logger) {
    if (logger) logger->error("vm45_select: 非 Windows 平台不支持 DLL 动态加载");
    return false;
}
    ...
    Logger* logger)
{
    ...
    // 像素尺度 (角秒/像素)
    s0 = VM45_ARCSEC_PER_UM_PER_MM * pixel_size_um / focal_length_mm;
    ...
    query_area_sqdeg = VM45_PI * query_radius_deg * query_radius_deg;
...
{
    double ra = ra_deg * VM45_DEGTORAD;
    double dec = dec_deg * VM45_DEGTORAD;
    double ra0 = ra0_deg * VM45_DEGTORAD;
    double dec0 = dec0_deg * VM45_DEGTORAD;
    ...
    // 转换为角秒
    xi_asec = valid ? xi_rad * VM45_ASEC_PER_RAD : 0.0;
    eta_asec = valid ? eta_rad * VM45_ASEC_PER_RAD : 0.0;
}
    ...
    Logger* logger)
{
    ...
    // --- 参数校验 ---
    if (focal_length_mm <= 0.0) {
        if (logger) logger->error("vm45_select: focal_length_mm 非法");
        return -1;
    }
    if (pixel_size_um <= 0.0) {
        if (logger) logger->error("vm45_select: pixel_size_um 非法");
        return -1;
    }
    if (image_path.empty()) {
        if (logger) logger->error("vm45_select: image_path 为空");
        return -1;
    }

    if (logger) logger->info("=== vm45_select 启动 ===");

    // --- 获取注入的句柄 ---
    void* gaia_handle = get_gaia_client_handle();
    void* detector_handle = get_star_detector_handle();
    if (!gaia_handle) {
        if (logger) logger->error("vm45_select: GaiaClient 句柄未注入");
        return -1;
    }
    if (!detector_handle) {
        if (logger) logger->error("vm45_select: StarDetector 句柄未注入");
        return -1;
    }

    // --- 加载依赖 DLL ---
    if (!load_dlls(logger)) {
        if (logger) logger->error("vm45_select: 依赖 DLL 加载失败");
        return -1;
    }
    ...
    if (!img_data) {
        if (logger) logger->error("vm45_select: 图像读取失败");
        return -1;
    }
    ...
    if (img_w <= 0 || img_h <= 0 || !pixel_data) {
        if (logger) logger->error("vm45_select: 图像数据非法");
    ...
    if (det_ret != 0 || det_count <= 0) {
        if (logger) logger->error("vm45_select: 星点检测失败或未检测到星");
    ...
    if (N < 2) {
        if (logger) logger->error("vm45_select: 图像侧选星数过少");
        g_dll.sdet_free_ex(det_x, det_y, det_flux, det_sat, nullptr, 0);
        return -1;
    }

    // --- Step 4: 构建 U 向量组 (角秒坐标, 原点在图像中心, Y 轴向上) ---
    double s0 = VM45_ARCSEC_PER_UM_PER_MM * pixel_size_um / focal_length_mm;
    ...
    output.s0 = s0;  // V4.5 新增: 供 Phase A 相对向量法使用
    ...
    if (q_ret != 0 || cat_ra.size() < 2) {
        if (logger) logger->error("vm45_select: Gaia 星表查询星数过少");
        return -1;
    }
    ...
    if (fov_idx.size() < 2) {
        if (logger) logger->error("vm45_select: FOV 内 Gaia 星数过少");
        return -1;
    }
...
#else
    // 非 Windows 平台不支持
    if (logger) logger->error("vm45_select: 非 Windows 平台不支持");
    return -1;
#endif // _WIN32

    output.success = true;
    if (logger) logger->info("=== vm45_select 完成 ===");
    return 0;
}
