...
static void test_convergence()
{
    ...
    // 初始 m_cut
    double m_cut = v43::compute_initial_mag_cut(200.0, 1.0, nullptr);

    // 迭代
    double m_final;
    int n_final, iterations;
    bool converged;
    v43::density_match_iterate(
...
static void test_small_fov()
{
    ...
    double m_cut = v43::compute_initial_mag_cut(1000.0, 1.0, nullptr);
    double m_final;
    int n_final, iterations;
    bool converged;
    v43::density_match_iterate(
...
static void test_initial_mag_cut()
{
    ...
    double m1 = v43::compute_initial_mag_cut(200.0, 1.0, nullptr);
    ...
    double m2 = v43::compute_initial_mag_cut(1000.0, 60.0, nullptr);
    ...
    // f=50mm, t=300s → 短焦长曝光
    double m3 = v43::compute_initial_mag_cut(50.0, 300.0, nullptr);
...
static void test_select_image_stars()
{
    ...
    // 场景 A: 饱和星 60 颗 > img_n_target=50 → 全选饱和星 (60颗)
    {
        ...
        auto sel = v43::select_image_stars(flux, sat, 50, nullptr);
...
static void test_gnomonic()
{
    ...
    // 中心点投影 → (0, 0)
    {
        double xi, eta;
        bool valid;
        v43::gnomonic_forward_proj(100.0, 30.0, 100.0, 30.0, xi, eta, valid);
    ...
    // 在赤道 (dec=0), RA 偏移 1" → xi ≈ 1" (cos(dec)=1)
    {
        ...
        // RA 偏移 1 角秒 = 1/3600 度
        v43::gnomonic_forward_proj(ra0 + 1.0/3600.0, dec0, ra0, dec0, xi, eta, valid);
    ...
    // Dec 方向偏移 1 角秒 → eta ≈ 1"
    {
        ...
        v43::gnomonic_forward_proj(ra0, dec0 + 1.0/3600.0, ra0, dec0, xi, eta, valid);
    ...
    // 天极附近投影应无效 (cosc < 0)
    {
        ...
        v43::gnomonic_forward_proj(280.0, 0.0, 100.0, 0.0, xi, eta, valid);
