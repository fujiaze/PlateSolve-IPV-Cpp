// ============================================================================
// test_fit.cpp - V4.3 WcsFitter (vm43_fit) 单元测试 (Task 7)
//
// 测试内容:
...
static void test_linear_cd_no_outliers() {
    ...
    // 运行 vm43_fit (跳过 SIP, 仅测试线性 CD)
    VM43SolveParams params;
...
static void test_fit_with_outliers() {
    ...
    int ret_huber = vm43_fit(data.U, data.W, data.pairs,
                             ra0, dec0, focal_length_mm, pixel_size_um,
                             img_width, img_height, 4, params, result_huber, nullptr);

    if (ret_huber != 0) {
        TEST_FAIL("含外点Huber", "vm43_fit (Huber) 返回失败");
        return;
    }
...
static void test_sip_order2_fit() {
    ...
    // 运行 vm43_fit (启用 SIP, max_order=4)
    VM43SolveParams params;
    std::memset(&params, 0, sizeof(params));
    params.skip_sip = 0;
    params.irm_sip_min_pairs = 30;
    ...
    WcsFitResult result;
    int ret = vm43_fit(U, W, pairs, ra0, dec0, focal_length_mm, pixel_size_um,
                       img_width, img_height, 4, params, result, nullptr);

    if (ret != 0) {
        TEST_FAIL("SIP2阶拟合", "vm43_fit 返回失败");
        return;
    }
    if (!result.success) {
...
int main() {
    printf("============================================================\n");
    printf("V4.3 WcsFitter (vm43_fit) 单元测试 - Task 7\n");
    printf("============================================================\n");
