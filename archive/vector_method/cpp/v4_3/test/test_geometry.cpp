...
static v43::VM43SolveParams make_params() {
...
static int test_true_match() {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        v43::StarPoint p = make_star(10.0 * std::cos(ang), 10.0 * std::sin(ang));
        U.push_back(p);
        W.push_back(p);
    }
    ...
    std::vector<v43::MatchPair> cand = { {0, 0} };
    std::vector<v43::MatchPair> filtered;
    v43::VM43SolveParams params = make_params();

    int ret = v43::vm43_geometry_filter(cand, U, W, /*s0=*/1.0, /*s_robust=*/0.0,
...
static int test_performance() {
    ...
    int ret = v43::vm43_geometry_filter(cand, U, W, 1.0, 0.0, params, filtered, nullptr);
