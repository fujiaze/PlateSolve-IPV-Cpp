// ============================================================================
// test_solve.cpp - V4.3 入口 vm43_solve() 单元测试
//
// 测试范围 (不依赖外部 GaiaClient/StarDetector):
//   1. 参数边界检查 (NULL result/image_path, 无效 focal/pixel)
//   2. 依赖注入检查 (未注入 Gaia/StarDetector)
//   3. 资源管理 (vm43_free_result NULL/空/已分配)
//   4. 默认参数 (vm43_get_default_params 字段正确)
//   5. 句柄注入 (vm43_set_gaia_client / vm43_set_star_detector)
//
// 完整端到端测试需要真实 GaiaClient + StarDetector, 在 Python 端进行
// ============================================================================

#include "../include/vm43_api.h"
#include "../include/vm43_types.h"
#include <cstring>
#include <cstdio>
#include <string>
...
static int test_null_result() {
    int rc = vm43_solve("dummy.fits", 10.0, 20.0, 1000.0, 5.0, nullptr, nullptr);
...
static int test_null_image_path() {
    ...
    int rc = vm43_solve(nullptr, 10.0, 20.0, 1000.0, 5.0, nullptr, &result);
...
static int test_invalid_focal_length() {
    v43::VM43SolveResult result;
    int rc = vm43_solve("dummy.fits", 10.0, 20.0, 0.0, 5.0, nullptr, &result);
...
static int test_invalid_pixel_size() {
    v43::VM43SolveResult result;
    int rc = vm43_solve("dummy.fits", 10.0, 20.0, 1000.0, -1.0, nullptr, &result);
...
static int test_no_gaia_client() {
    // 确保未注入任何句柄
    vm43_set_gaia_client(nullptr);
    vm43_set_star_detector(nullptr);

    v43::VM43SolveResult result;
    int rc = vm43_solve("dummy.fits", 10.0, 20.0, 1000.0, 5.0, nullptr, &result);
...
static int test_no_star_detector() {
    ...
    vm43_set_gaia_client((void*)0xDEADBEEF);
    vm43_set_star_detector(nullptr);
...
// ============================================================================
// 测试 7: vm43_get_default_params 字段正确
// ============================================================================
static int test_get_default_params() {
    v43::VM43SolveParams params;
    vm43_get_default_params(&params);
...
// ============================================================================
// 测试 8: vm43_free_result(NULL) 安全
// ============================================================================
static int test_free_null_result() {
    vm43_free_result(nullptr);  // 不应崩溃
    return 0;
}

// ============================================================================
// 测试 9: vm43_free_result 对未分配 cu/cw 的 result 安全
// ============================================================================
...
// ============================================================================
// 测试 10: vm43_free_result 对已分配 cu/cw 的 result 正确释放
// ============================================================================
static int test_free_allocated_result() {
    v43::VM43SolveResult result;
    std::memset(&result, 0, sizeof(result));

    // 模拟内部分配 (与 vm43_entry.cpp 中相同的方式)
    result.n_pairs = 3;
    result.cu = new int[3];
    result.cw = new int[3];
...
// ============================================================================
// 测试 11: vm43_free_result 可安全重复调用
// ============================================================================
static int test_free_idempotent() {
    ...
    vm43_free_result(&result);
    vm43_free_result(&result);  // 第二次调用不应崩溃
    vm43_free_result(&result);  // 第三次也不应崩溃
