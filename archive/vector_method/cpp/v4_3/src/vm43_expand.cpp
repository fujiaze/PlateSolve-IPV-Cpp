// ============================================================================
// vm43_expand.cpp - V4.3 PairExpander 模块 (Phase C / IRM Step 1)
//
...
// 与 vm43_score.cpp 中的实现一致, 因其位于匿名命名空间无法直接复用
    ...
    Logger* logger)
{
    ...
    if (N <= 0 || M <= 0) {
        if (logger) logger->warn("vm43_expand: U/W 为空");
        return -1;
    }
    ...
    if (!has_inv && logger) {
        logger->warn("vm43_expand: CD 矩阵奇异, 跳过双向验证");
    }
