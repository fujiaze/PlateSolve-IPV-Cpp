...
//   循环 iter = 1..max_iter:
//     Step 1: vm43_expand(U, W, CD, SIP, S_robust, ...) → 候选匹配对
//     Step 2: vm43_geometry_filter(candidates, ...) → 几何过滤后对
//     Step 3: vm43_verify(filtered, ...) → RANSAC + 贝叶斯 + 三角形 → 内点集
//     Step 4: 合并 C0 ∪ inliers → control_points (去重)
//     Step 5: vm43_fit(control_points, ...) → CD + SIP + RMS
//     Step 6: S_robust_new = vm43_compute_s_robust(...)
//     收敛判定:
    ...
    Logger* logger)
{
    ...
    // --- 边界检查 ---
    if (C0.empty()) {
        if (logger) logger->warn("vm43_irm: C0 为空, 无法启动 IRM");
        return -1;
    }
    if (U.empty() || W.empty()) {
        if (logger) logger->warn("vm43_irm: U 或 W 为空");
        return -1;
    }
    if (focal_length_mm <= 0 || pixel_size_um <= 0) {
        if (logger) logger->warn("vm43_irm: focal_length_mm 或 pixel_size_um 无效");
        return -1;
    }
    ...
    if (logger) logger->infof("=== vm43_irm 开始 (M0=%d, max_iter=%d, s0=%.4f\") ===",
                               M0, max_iter, s0);
    ...
    double s_robust;
    int rc = vm43_compute_s_robust(U, W, C0, CD, SIP, s0, M0, s_robust_result, logger);
    if (rc != 0 || s_robust_result.s_robust <= 0) {
        // 兜底: 用 s0 × 0.1 作为初始评分
        s_robust = s0 * 0.1;
        if (logger) logger->warnf("vm43_irm: 初始 S_robust 计算失败, 兜底 s_robust=%.4f\"", s_robust);
    } else {
        s_robust = s_robust_result.s_robust;
    }

    if (logger) logger->infof("vm43_irm 初始: S_robust=%.4f\", N_inliers=%d",
    ...
    for (int iter = 1; iter <= max_iter; ++iter) {
        ...
        rc = vm43_expand(U, W, CD, SIP, s_robust, s0, ra0, dec0,
                         img_width, img_height, params, expand_result, logger);
        if (rc != 0 || expand_result.candidates.empty()) {
            if (logger) logger->warnf("vm43_irm 迭代 %d: 扩增失败, 使用 C0", iter);
            expand_result.candidates = C0;
        }

        // ═══ Step 2: 局部几何一致性过滤 ═══
        std::vector<MatchPair> filtered;
        rc = vm43_geometry_filter(expand_result.candidates, U, W, s0, s_robust,
                                   params, filtered, logger);
        if (rc != 0 || filtered.empty()) {
            if (logger) logger->warnf("vm43_irm 迭代 %d: 几何过滤后为空, 使用 C0", iter);
            filtered = C0;
        }

        // ═══ Step 3: 验证 (MAD + RANSAC + 贝叶斯 + 三角形) ═══
        VerificationResult verify_result;
        rc = vm43_verify(filtered, U, W, s0, fov_diag_deg, params,
                          verify_result, logger);
        if (rc != 0 || verify_result.inliers.empty()) {
            if (logger) logger->warnf("vm43_irm 迭代 %d: 验证失败, 使用 C0", iter);
        ...
        if (logger) logger->infof("vm43_irm 迭代 %d: 控制点 %d → %d (C0=%d + inliers=%d)",
                                   iter, n_old, n_new, M0, (int)verify_result.inliers.size());

        // 控制点数未增长且已迭代 ≥ 2 次 → 收敛
        if (n_new <= n_old && iter >= 2) {
            if (logger) logger->infof("vm43_irm 迭代 %d: 控制点不再增长, 收敛", iter);
        ...
        rc = vm43_compute_s_robust(U, W, control_points, CD, SIP, s0, M0,
                                    s_robust_new_result, logger);
        if (rc != 0 || s_robust_new_result.s_robust <= 0) {
            if (logger) logger->warnf("vm43_irm 迭代 %d: S_robust 计算失败, 保留上一轮", iter);
            n_iters = iter;
            continue;
        }
        double s_robust_new = s_robust_new_result.s_robust;

        if (logger) logger->infof("vm43_irm 迭代 %d: S_robust %.4f → %.4f (Δ=%.4f, n_inliers=%d)",
        ...
        if (delta < converge_eps) {
            if (logger) logger->infof("vm43_irm 迭代 %d: 收敛 (Δ=%.4f < %.4f)",
        ...
        if (s_robust_new > s_robust * diverge_factor) {
            if (logger) logger->warnf("vm43_irm 迭代 %d: S_robust 变差 (%.4f > %.4f×%.4f), 停止",
    ...
    if (logger) logger->infof("=== vm43_irm 完成: iter=%d, converged=%d, S_robust=%.4f\", N=%d ===",
