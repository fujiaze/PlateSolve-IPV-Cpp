// ============================================================================
// vm43_match.cpp - V4.3 VectorMatcher 模块 (Phase A+B)
//
...
//   - namespace v42 → v43
//   - 接口改为内部 C++ 函数 v43::vm43_match (无 ctypes 边界, 无 JSON 序列化)
    ...
    Logger& logger)
{
    ...
    for (int iter = 0; iter < Kmax; ++iter) {
        ...
        double td = wrap180(theta * VM43_RADTODEG);
        double ct = std::cos(theta), st = std::sin(theta);
    ...
    logger.info("PhaseB OK: s=" + std::to_string(res.s) +
                " θ=" + std::to_string(res.theta * VM43_RADTODEG) + "°" +
    ...
    Logger* logger)
{
    ...
    if (N < 2 || M < 2) {
        log.error("vm43_match: N 或 M 过少 (N=" + std::to_string(N) +
                  " M=" + std::to_string(M) + ")");
    ...
    for (int m = 0; m < n_modes; ++m) {
        mres[m] = solve_single_mode(U.data(), N, W.data(), M, m, s0,
    ...
    log.info("=== vm43_match 完成 === mode=" + std::to_string(best_mode) +
             " s=" + std::to_string(best.s) +
             " θ=" + std::to_string(best.theta * VM43_RADTODEG) + "°" +
