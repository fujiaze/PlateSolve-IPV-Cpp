// ============================================================================
// vm43_geometry.cpp - V4.3 局部几何一致性过滤模块 (IRM Step 2)
//
    ...
    Logger* logger)
{
    ...
    if (candidates.empty()) {
        if (logger) logger->warn("vm43_geometry_filter: 候选为空, 无需过滤");
        return 0;
    }
    if (U.empty() || W.empty()) {
        if (logger) logger->error("vm43_geometry_filter: U 或 W 为空");
        return -1;
    }
    ...
    if (logger) {
        logger->infof("vm43_geometry_filter: 候选=%d, K_img=%d, K_gaia=%d, "
    ...
    for (int ci = 0; ci < (int)candidates.size(); ++ci) {
        ...
        if (mp.u < 0 || mp.u >= (int)U.size() ||
            ...
            if (logger) logger->warnf("vm43_geometry_filter: 候选 %d 索引越界 (u=%d,w=%d)",
                                      ci, mp.u, mp.w);
            continue;
        }
    ...
    if (logger) {
        logger->infof("vm43_geometry_filter: 保留=%d, 丢弃=%d, 耗时=%.2f ms",
                      n_kept, n_dropped, ms);
    }
