    ...
    int n_img_bright, double k_match, double query_radius_factor)
{
    ...
    if (focal_length_mm <= 0.0) {
        std::fprintf(stderr, "[vm4_1_density] 警告: focal_length_mm=%.4f 非法, 返回零值\n",
                     focal_length_mm);
        return info;
    }
    ...
    std::fprintf(stderr,
        "[vm4_1_density] FOV计算: s0=%.4f\"/px, FOV_diag=%.4f°, query_r=%.4f°, "
...
double compute_initial_mag_cut(double focal_length_mm, double exposure_time_s)
{
    ...
    std::fprintf(stderr,
        "[vm4_1_density] 初始星等: m_cut=%.4f (f=%.2fmm, t=%.2fs)\n",
    ...
    std::function<int(double,double,double,double)> gaia_query_func)
{
    ...
    if (!gaia_query_func) {
        std::fprintf(stderr, "[vm4_1_density] 错误: gaia_query_func 为空\n");
        return result;
    }
    if (n_target <= 0) {
        std::fprintf(stderr, "[vm4_1_density] 警告: n_target=%d 非正, 跳过迭代\n", n_target);
        return result;
    }
    ...
    std::fprintf(stderr,
        "[vm4_1_density] 迭代开始: ra=%.4f, dec=%.4f, r=%.4f°, N_target=%d, "
    ...
    for (i = 0; i < max_iter; ++i) {
        ...
        std::fprintf(stderr,
            "[vm4_1_density] iter=%d  m_lim=%.3f  n_gaia=%d  (target=%d, 范围=[%.1f,%.1f])\n",
    ...
    std::fprintf(stderr,
        "[vm4_1_density] 迭代结束: %s  m_final=%.3f  n_final=%d  iters=%d\n",
    ...
    double center_ra, double center_dec, double fov_diag_deg)
{
    ...
    std::fprintf(stderr,
        "[vm4_1_density] 投影: 输入=%zu 颗, FOV内=%d 颗 (center=(%.4f,%.4f), r=%.2f\")\n",
...
// ----------------------------------------------------------------------------
// V4.1 compute_fov_and_density_asym
// ----------------------------------------------------------------------------
    ...
    int n_img_bright, double gaia_density_ratio, double gaia_query_radius_factor)
{
    FovDensityInfo info{};
    if (focal_length_mm <= 0.0) {
        std::fprintf(stderr, "[vm4_1_density] 警告: focal_length_mm=%.4f 非法\n", focal_length_mm);
        return info;
    }
    ...
    // 图像面积(平方度)
    double img_area_sqdeg = (img_width * info.s0 / 3600.0) * (img_height * info.s0 / 3600.0);
    if (img_area_sqdeg <= 0.0) img_area_sqdeg = info.query_area_sqdeg;

    // 图像面密度
    info.rho_img = (img_area_sqdeg > 0.0) ? static_cast<double>(n_img_bright) / img_area_sqdeg : 0.0;

    // V4.1: Gaia目标密度 = gaia_density_ratio × 图像密度
    info.rho_target = gaia_density_ratio * info.rho_img;

    // V4.1: 目标星数 = gaia_density_ratio × n_img × (查询圆面积/图像面积)
    double n_target_dbl = gaia_density_ratio * static_cast<double>(n_img_bright)
                        * (info.query_area_sqdeg / std::max(img_area_sqdeg, 1e-10));
    info.n_target = std::max(50, static_cast<int>(std::lround(n_target_dbl)));

    std::fprintf(stderr,
        "[vm4_1_density] V4.1不对称密度: s0=%.4f\"/px, FOV_diag=%.4f°, query_r=%.4f°, "
        "img_area=%.5f°², query_area=%.5f°², rho_img=%.2f, rho_target=%.2f, n_target=%d\n",
        info.s0, info.fov_diag_deg, info.query_radius_deg,
        img_area_sqdeg, info.query_area_sqdeg, info.rho_img, info.rho_target, info.n_target);

    return info;
}

// ----------------------------------------------------------------------------
// V4.1 density_match_query_asym (自适应步长)
// ----------------------------------------------------------------------------
    ...
    std::function<int(double,double,double,double)> gaia_query_func)
{
    ...
    if (!gaia_query_func || n_target <= 0) {
        std::fprintf(stderr, "[vm4_1_density] V4.1错误: gaia_query_func空或n_target=%d\n", n_target);
        return result;
    }
    ...
    std::fprintf(stderr,
        "[vm4_1_density] V4.1迭代开始: ra=%.4f, dec=%.4f, r=%.4f°, N_target=%d, "
    ...
    for (i = 0; i < max_iter; ++i) {
        ...
        // V4.1自适应步长: 前4次step_init，后续step_init/2
        double step = (i < 4) ? step_init : step_init * 0.5;

        std::fprintf(stderr,
            "[vm4_1_density] V4.1 iter=%d  m_lim=%.3f  n_gaia=%d  (target=%d, 范围=[%.1f,%.1f], step=%.3f)\n",
    ...
    std::fprintf(stderr,
        "[vm4_1_density] V4.1迭代结束: %s  m_final=%.3f  n_final=%d  iters=%d\n",
        converged ? "收敛" : "未收敛(达到max_iter)",
        result.final_mag_lim, result.final_n_gaia, result.iterations);
