    ...
    int prosac_T_max, int use_prosac)
{
    ...
    fprintf(stderr,"[vm4_1] PhaseA 稀疏度: U中位=%.1f\" W中位=%.1f\"\n",sp_med_u,sp_med_w);
    vm4_1_log::debug("PhaseA 稀疏度: U中位=" + std::to_string(sp_med_u) + " W中位=" + std::to_string(sp_med_w));
    ...
    if (use_prosac_local) {
        ...
        fprintf(stderr,"[vm4_1] PhaseA PROSAC已启用: N=%d T_max=%d quality_median=%.4f\n",
                N, prosac_T_max, res.prosac_quality_median);
        vm4_1_log::info("PhaseA PROSAC 启用: N=" + std::to_string(N) +
                      " T_max=" + std::to_string(prosac_T_max) +
                      " quality_median=" + std::to_string(res.prosac_quality_median));
    } else {
        fprintf(stderr,"[vm4_1] PhaseA PROSAC已禁用，退化为V3.5稀疏度加权抽样\n");
        vm4_1_log::info("PhaseA PROSAC 禁用，退化为V3.5稀疏度加权抽样");
    }
    ...
    for(int iter=0;iter<Kmax;++iter){
        ...
        if(nr>best_n){
            best_n=nr;
            fprintf(stderr,"[vm4_1] PhaseA: new best_n=%d (i=%d j=%d s=%.4f θ=%.2f° tx=%.1f\" ty=%.1f\")\n",
                    nr,i,j,s,td,tx,ty);
        }
        n_val++;
        ...
        if(n_val>=min_samples&&iter-last_snr>=batch_size){
            ...
            fprintf(stderr,"[vm4_1] PhaseA: n=%d best=%d peak=%.0f θ=%.2f° SNR=%.1fx (5N=%.0f 10N=%.0f)\n",
                    n_val,best_n,th_hist[snr.peak_idx],snr.peak_deg,snr.snr,t5,t10);
            if(snr.snr>=t10){fprintf(stderr,"[vm4_1] PhaseA: ≥10N stop\n");break;}
            if(snr.snr>=t5){fprintf(stderr,"[vm4_1] PhaseA: ≥5N stop\n");break;}
        }
    }
    // 记录 PROSAC 最终采样池大小
    if (use_prosac_local) {
    ...
    fprintf(stderr,"[vm4_1] PhaseA done: n=%d best=%d med=%.1f pk=%.0f θ=%.2f° SNR=%.1fx (5N=%.0f) rec=%zu\n",
    ...
    // 调试: θ直方图top-5峰值
    {
        ...
        fprintf(stderr,"[vm4_1] PhaseA θ直方图top5:");
    ...
    // 调试: n_in_range分布 (P25/P50/P75/P90/P95/P99/max)
    {
        ...
        fprintf(stderr,"[vm4_1] PhaseA n_in_range分布: min=%d P25=%d P50=%d P75=%d P90=%d P95=%d P99=%d max=%d\n",
    ...
    // 调试: θ峰值±2°内的records统计
    {
        ...
        fprintf(stderr,"[vm4_1] PhaseA θ峰值±2°: %d records (n≥3: %d), 平均n_in_range=%.1f\n",
    ...
    // 调试: 对best_n最高的record做详细变换分析
    {
        ...
        if(best_rec){
            ...
            fprintf(stderr,"[vm4_1] PhaseA best_record分析: i=%d j=%d s=%.4f θ=%.2f°\n",bi,bj,s,best_rec->theta_deg);
            fprintf(stderr,"[vm4_1]   N=%d M=%d max_dist=%.1f\" s_range=[%.2f,%.2f]\n",N,M,5.0*s0,s_min,s_max);
            fprintf(stderr,"[vm4_1]   距离OK: %d/%d  scaleOK: %d/%d  两者OK: %d/%d\n",
                    n_dist_ok,N,n_scale_ok,N,n_match,N);
            if(!match_dists.empty()){
                std::sort(match_dists.begin(),match_dists.end());
                fprintf(stderr,"[vm4_1]   匹配距离: min=%.2f\" P50=%.2f\" max=%.2f\"\n",
    ...
    if(filt.size()<2){fprintf(stderr,"[vm4_1] PhaseB: <2 pairs\n");return res;}
    ...
    res.cu = cu; res.cw = cw;
    fprintf(stderr,"[vm4_1] PhaseB: %zu corr\n", cu.size());
    if(cu.size()<2){fprintf(stderr,"[vm4_1] PhaseB: <2 corr\n");return res;}
    ...
    if(!sim.valid){fprintf(stderr,"[vm4_1] PhaseB: SVD invalid\n");return res;}
    ...
    // 调试: SVD变换后的匹配距离分布
    {
        ...
        if(!dists.empty()){
            ...
            fprintf(stderr,"[vm4_1] PhaseB SVD后距离: n_near=%zu min=%.2f\" P25=%.2f\" P50=%.2f\" P75=%.2f\" max=%.2f\" (tau=%.2f\")\n",
                    dists.size(),dists.front(),dists[dists.size()/4],dists[dists.size()/2],
                    dists[3*dists.size()/4],dists.back(),1.0*s0);
        } else {
            fprintf(stderr,"[vm4_1] PhaseB SVD后: 无近邻 (5*s0=%.1f\")\n",5.0*s0);
        }
    }
    ...
    fprintf(stderr,"[vm4_1] PhaseB OK: s=%.4f θ=%.2f° n=%d rms=%.3f corr=%zu\n",
    ...
    const double* norm_U, const double* norm_Wf)
{
    ...
    fprintf(stderr, "[vm4_1] PhaseC: %zu NN pairs\n", candidates.size());
    ...
    fprintf(stderr, "[vm4_1] PhaseC: %zu expanded (1to1)\n", expanded.size());
    return expanded;
}
    ...
    const std::vector<Pair>& expanded, double s0)
{
    ...
    do {
        ...
        // V3.5: 保留数 < 10时停止迭代
        if (n_keep < 10) {
            fprintf(stderr, "[vm4_1] PhaseD: 保留数 %d < 10, 停止迭代\n", n_keep);
            break;
        }
        ...
        if (removal_rate > 0.3) {
            fprintf(stderr, "[vm4_1] PhaseD 警告: 累计剔除率 %.1f%% > 30%% (已剔除 %d/%d)\n",
    ...
    fprintf(stderr, "[vm4_1] PhaseD: %d removed in %d iters, %zu clean pairs, MAD-RMS=%.3f\"\n",
    ...
    int n_bins, int fit_order, int n_iters)
{
    ...
    // Step 2: 方向一致性过滤
    {
        ...
        fprintf(stderr, "[vm4_1] PhaseD' 方向过滤: %d → %d\n", n, n_dir);
    }
    ...
    for (int it = 0; it < n_iters; ++it) {
        ...
        fprintf(stderr, "[vm4_1] PhaseD' 径向幅度过滤 iter %d: %d 保留\n", it, n_after);
    }
    ...
    int radial_n_bins, int radial_fit_order, int radial_n_iters)
{
    ...
    fprintf(stderr, "[vm4_1] PhaseD' 正向NN距离: med=%.2f min=%.2f max=%.2f\n",
    ...
    fprintf(stderr, "[vm4_1] PhaseD' 双向NN: %d 互为最近邻对 (n_gaia=%d n_img=%d)\n",
            er.n_mutual, n_gaia_use, n_img_use);

    if (mutual_pairs.size() < 5) {
        fprintf(stderr, "[vm4_1] PhaseD' 互为最近邻对太少，跳过扩增\n");
        return er;
    }
    ...
    fprintf(stderr, "[vm4_1] PhaseD' 径向过滤后: %d 对\n", er.n_after_filter);
    ...
    fprintf(stderr, "[vm4_1] PhaseD' 扩增完成: %zu 对 (1to1去重后)\n", er.expand_u.size());
    return er;
}
    ...
    const char* wcs_out_path)
{
    ...
    std::vector<int> clean_w_f = clean_w;
    {
        ...
        if(pre_sim.valid){
            ...
            fprintf(stderr,"[vm4_1] Pre-filter(向量中值离群): M_D=%d → %zu 对 (med_dx=%.3f\" σ_dx=%.3f\" med_dy=%.3f\" σ_dy=%.3f\")\n",
                    n_pre,clean_u_f.size(),mdx,sdx,mdy,sdy);
            M_D = (int)clean_u_f.size();
        } else {
            fprintf(stderr,"[vm4_1] Pre-filter: umeyama invalid (M_D=%d), skip\n", M_D);
        }
    }
    ...
    if (M_D < 2) {
        ...
        fprintf(stderr, "[vm4_1] PhaseE: M_D=%d<2, fallback PhaseB CD\n", M_D);
        if(wcs_out_path&&wcs_out_path[0]){
            ...
            if(fw){
                ...
                fprintf(fw,"]\n}\n");
                fclose(fw);
                fprintf(stderr,"[vm4_1] WCS JSON written to: %s\n",wcs_out_path);
            }
        }
        return sr;
    }
    ...
    fprintf(stderr,"[vm4_1] Umeyama: s=%.6f θ=%.4f° CD_RMS=%.4fpx\n",
    ...
    for(int mad_iter=0;mad_iter<3;++mad_iter){
        ...
        fprintf(stderr,"[vm4_1] MAD轮%d: keep=%zu thresh=%.1fpx removed=%d\n",
    ...
    if(M_clean<5){fprintf(stderr,"[vm4_1] MAD后点数不足\n");return sr;}
    ...
    fprintf(stderr,"[vm4_1] 仿射CRVAL更新: center_ra=%.7f crval=%.7f Δ=%.6fdeg tx=%.1fpx\n",
    ...
    fprintf(stderr,"[vm4_1] 仿射(MAD后%d点): A=[%.6f,%.6f;%.6f,%.6f] t=[%.3f,%.3f]px RMS=%.4fpx\n",
    ...
    for(int try_o=2;try_o<=max_order;++try_o){
        ...
        fprintf(stderr,"[vm4_1] SIP阶=%d 项=%d RMS=%.4fpx BIC=%.2f\n",try_o,nhi,rms_h,bic);
        if(bic<best_bic){
    ...
    fprintf(stderr,"[vm4_1] PhaseE: Ume_RMS=%.3f affine_RMS=%.3f SIP阶=%d RMS=%.3fpx s=%.6f θ=%.3f°\n",
            rms_best,rms_aff,best_order,best_rms,sr.s_refined,sr.theta_refined*RADTODEG);
    fprintf(stderr,"[vm4_1] CD=[[%.6e,%.6e],[%.6e,%.6e]] CRVAL=[%.10f,%.10f]\n",
            sr.cd[0],sr.cd[1],sr.cd[2],sr.cd[3],sr.crval[0],sr.crval[1]);

    // SIP系数打印
    int ndb=std::min(10,Mc);
    fprintf(stderr,"[vm4_1] SIP coeffs(first %d non-zero,order=%d):\n",ndb,sr.order);
    int np=0;
    for(int p=0;p<6&&np<ndb;++p)for(int q=0;q<6&&np<ndb;++q){
        ...
        fprintf(stderr,"[vm4_1]   A[%d][%d]=%.6e  B[%d][%d]=%.6e\n",p,q,sr.sip_A[p*6+q],p,q,sr.sip_B[p*6+q]);
        np++;
    }
    } else {
        sr.order=0; sr.rms_px=rms_aff;
        fprintf(stderr,"[vm4_1] PhaseE: Ume_RMS=%.3f affine_RMS=%.3f 跳过SIP(order=0) s=%.6f θ=%.3f°\n",
    ...
    if(wcs_out_path&&wcs_out_path[0]){
        ...
        if(fw){
            ...
            fclose(fw);
            fprintf(stderr,"[vm4_1] WCS JSON written to: %s\n",wcs_out_path);
        }else fprintf(stderr,"[vm4_1] ERROR: cannot open WCS output: %s\n",wcs_out_path);
    }
                           ...
                           double center_ra, double center_dec)
{
    ...
    if(early_exit && early_exit->load(std::memory_order_relaxed)){fprintf(stderr,"[vm4_1] mode%d: skip\n",mode);return mr;}
    std::vector<double> Wf(M*2); apply_flip(W,M,mode,Wf.data());
    fprintf(stderr,"[vm4_1] mode%d: N=%d M=%d\n",mode,N,M);
    vm4_1_log::info("mode" + std::to_string(mode) + ": N=" + std::to_string(N) + " M=" + std::to_string(M));
    ...
    if(!mr.ab.success){fprintf(stderr,"[vm4_1] mode%d: fail\n",mode);vm4_1_log::warn("mode" + std::to_string(mode) + " Phase A/B 失败");return mr;}
    ...
    {
        vm4_1_log::info("mode" + std::to_string(mode) + " Phase C: 变换驱动 NN 扩充开始");
        ...
        fprintf(stderr,"[vm4_1] mode%d PhaseC范围过滤: %d/%d Gaia星在图像外, %d进入KDTree查询\n",
        ...
        fprintf(stderr,"[vm4_1] mode%d PhaseC: 候选 %zu → 贝叶斯接受 %zu (拒%d) → 选取 %zu, 合计 %d 对"
        ...
        vm4_1_log::info("mode" + std::to_string(mode) + " PhaseC: 候选" +
    ...
    // ========================================================================
    if (phase_b_u.size() >= 3) {
        vm4_1_log::info("mode" + std::to_string(mode) + " Phase D: MAD 清洗开始, 输入 " +
        ...
        for (; iter < 3; ++iter) {
            ...
            fprintf(stderr,"[vm4_1] mode%d PhaseD 轮%d: keep=%zu thresh=%.2f\" removed=%d (med=%.2f σ=%.2f)\n",
                    mode, iter,
                    std::count(keep.begin(), keep.end(), true), thresh, n_removed, med, sigma);
            vm4_1_log::debug("mode" + std::to_string(mode) + " PhaseD 轮" + std::to_string(iter) +
        ...
        fprintf(stderr,"[vm4_1] mode%d PhaseD: %d iters, %d clean pairs, MAD-RMS=%.3f\"\n",
                mode, mr.n_phased_iterations, mr.n_phased_clean, mr.mad_rms_arcsec);
        vm4_1_log::info("mode" + std::to_string(mode) + " PhaseD: " +
        ...
        fprintf(stderr,"[vm4_1] mode%d PhaseD: 跳过 (<3 对)\n", mode);
    }
    ...
    if (mr.final_u.size() >= 3) {
        ...
        if (use_bayes) {
            ...
            fprintf(stderr,"[vm4_1] mode%d PhaseD' 贝叶斯: lnK=%.2f n=%d RMS=%.3f\" decision=%d (σ=%.2f A_fov=%.2f sqdeg)\n",
                    mode, br.lnK, br.n_match, br.rms_arcsec, br.decision, sigma, A_fov_sqdeg);
            vm4_1_log::info("mode" + std::to_string(mode) + " PhaseD' 贝叶斯: lnK=" +
                          std::to_string(br.lnK) + " decision=" + std::to_string(br.decision));
            if (br.decision < 0) {
                bayes_ok = false;
                vm4_1_log::warn("mode" + std::to_string(mode) + " PhaseD' 贝叶斯拒绝 (lnK < lnK_weak)");
            }
        }
        ...
        if (use_triangle) {
            ...
            fprintf(stderr,"[vm4_1] mode%d PhaseD' 三角形: total=%d passed=%d ratio=%.3f accepted=%d\n",
                    mode, tr.total_triangles, tr.passed, tr.pass_ratio, tr.accepted ? 1 : 0);
            vm4_1_log::info("mode" + std::to_string(mode) + " PhaseD' 三角形: ratio=" +
                          std::to_string(tr.pass_ratio));
            if (!tr.accepted) {
                tri_ok = false;
                vm4_1_log::warn("mode" + std::to_string(mode) + " PhaseD' 三角形拒绝 (ratio < " +
                              std::to_string(triangle_pass_rate) + ")");
            }
        }

        mr.validated = bayes_ok && tri_ok;
        fprintf(stderr,"[vm4_1] mode%d PhaseD' 验证: %s\n",
    ...
    if(mr.success&&mr.s>=0.9&&mr.s<=1.1&&mr.n_inliers>=min_inliers&&early_exit){early_exit->store(true,std::memory_order_relaxed);fprintf(stderr,"[vm4_1] mode%d converged (n=%d≥%d)\n",mode,mr.n_inliers,min_inliers);}
    fprintf(stderr,"[vm4_1] mode%d final: s=%.4f θ=%.2f° n=%d rms=%.3f norm=%.4f validated=%d\n",
            mode,mr.s,mr.theta*RADTODEG,mr.n_inliers,mr.rms,mr.norm_score,mr.validated?1:0);
    vm4_1_log::info("mode" + std::to_string(mode) + " final: s=" + std::to_string(mr.s) +
    ...
    const VM4_1SolveParams* params, VM4_1SolveResult* result)
{
    ...
    if (params->log_file_path && params->log_file_path[0]) {
        vm4_1_log::init(std::string(params->log_file_path));
        log_initialized = true;
        vm4_1_log::info("=== vm4_1_solve 开始 ===");
        vm4_1_log::info("N_img=" + std::to_string(N_img) + " M=" + std::to_string(M) +
                      " s0=" + std::to_string(params->s0) +
                      " fov_diag=" + std::to_string(params->fov_diag_asec) + "\"");
    }

    if (N_img < 2 || M < 2) {
        fprintf(stderr, "[vm4_1] N=%d M=%d too few\n", N_img, M);
        vm4_1_log::error("N_img 或 M 过少: N=" + std::to_string(N_img) + " M=" + std::to_string(M));
        if (log_initialized) vm4_1_log::close();
        return -1;
    }

    // ========================================================================
    // Phase 0: 密度查询 debug 信息记录
    //   实际查询由 Python 端完成（Task 9），C++ 端通过 VM4_1DebugInfo 记录
    //   这里仅在日志中记录传入的密度相关参数，供后续分析
    //   V4.1: 改用 compute_fov_and_density_asym 不对称密度匹配
    // ========================================================================
    vm4_1_log::info("Phase 0 参数: gaia_density_ratio=" + std::to_string(params->gaia_density_ratio) +
                  " gaia_query_radius_factor=" + std::to_string(params->gaia_query_radius_factor) +
                  " img_n_target=" + std::to_string(params->img_n_target) +
                  " n_img_bright=" + std::to_string(params->n_img_bright) +
                  " focal_length_mm=" + std::to_string(params->focal_length_mm) +
                  " m_lim_step=" + std::to_string(params->m_lim_step) +
                  " m_lim_max_iter=" + std::to_string(params->m_lim_max_iter));
    ...
    if (params->focal_length_mm > 0 && params->pixel_size_um > 0 &&
        ...
        // V4.1: 调用不对称密度匹配函数
        //   - gaia_density_ratio: Gaia面密度/图像面密度 (默认1.5)
        //   - gaia_query_radius_factor: Gaia查询半径因子 (默认0.55, 直径=1.1×FOV对角线)
        auto fov_info = compute_fov_and_density_asym(params->focal_length_mm, params->pixel_size_um,
                                                       params->img_width, params->img_height,
                                                       params->n_img_bright,
                                                       params->gaia_density_ratio,
                                                       params->gaia_query_radius_factor);
        rho_img_est = fov_info.rho_img;
        rho_target_est = fov_info.rho_target;
        vm4_1_log::info("Phase 0 V4.1估算: s0=" + std::to_string(fov_info.s0) +
                      " fov_diag=" + std::to_string(fov_info.fov_diag_deg) + "°" +
                      " query_r=" + std::to_string(fov_info.query_radius_deg) + "°" +
    ...
    vm4_1_log::info("开始 " + std::to_string(n_modes) + " 模式并行求解");
    ...
    for (int m = 0; m < n_modes; ++m) {
        ...
        fprintf(stderr,"[vm4_1] mode%d 评分: lnK=%.2f rms=%.3f\" score=%.2f validated=%d\n",
                m, mres[m].bayes_lnK, mres[m].mad_rms_arcsec, score, mres[m].validated?1:0);
        vm4_1_log::info("mode" + std::to_string(m) + " 评分: lnK=" +
    ...
    if (chosen_mode < 0) {
        fprintf(stderr, "[vm4_1] all modes failed\n");
        vm4_1_log::error("所有模式均失败");
        if (log_initialized) vm4_1_log::close();
        return -1;
    }

    if (!chosen_validated) {
        fprintf(stderr, "[vm4_1] 警告: 所有模式均未通过贝叶斯/三角形验证, 选评分最高 mode%d (fallback)\n",
                chosen_mode);
        vm4_1_log::warn("所有模式均未通过验证, fallback 选 mode" + std::to_string(chosen_mode));
    }
    ...
    if (n_pairs < 2) {
        ...
        fprintf(stderr, "[vm4_1] PhaseD clean pairs=%zu (<2), fallback linear only\n", n_pairs);
        vm4_1_log::warn("匹配对不足2对, 纯线性回退");
        if (log_initialized) vm4_1_log::close();
        return 0;
    }

    fprintf(stderr,"[vm4_1] PhaseD清洗后匹配对: %zu 对 → 分层拟合\n", n_pairs);
    vm4_1_log::info("PhaseD 清洗后匹配对: " + std::to_string(n_pairs) + " 对 → 分层拟合");
    ...
    fprintf(stderr, "[vm4_1] OK: mode=%d s=%.6f(Δ=%.4f%%) θ=%.2f° n=%zu SNR=%.1fx "
    ...
    vm4_1_log::info("=== vm4_1_solve 完成 === mode=" + std::to_string(chosen_mode) +
...
extern "C" VM4_1_API int vm4_1_count_inliers(
    const double* U, int N_img, const double* W, int M,
    double s, double theta, double tx, double ty,
