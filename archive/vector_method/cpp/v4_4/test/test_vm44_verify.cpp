// ============================================================================
// test_verify.cpp - vm44_verify 单元测试 (Task 5)
//
// 测试用例:
...
static bool test_empty() {
    ...
    int rc = vm44_verify(candidates, U, W, 1.0, 1.0, p, result, &logger);
    if (rc != -1) {
...
static bool test_perfect_match() {
    ...
    int rc = vm44_verify(candidates, U, W, 1.0, 2.0, p, result, &logger);
    if (rc != 0) {
        printf("  失败: 期望 rc=0, 实际 rc=%d\n", rc);
        return false;
    }
...
int main() {
    printf("================================\n");
    printf("vm44_verify 单元测试 (Task 5)\n");
    printf("================================\n\n");
    ...
    run(v44::test_empty);
    run(v44::test_too_few);
    run(v44::test_perfect_match);
    run(v44::test_with_outliers);
    run(v44::test_random_mismatch);
