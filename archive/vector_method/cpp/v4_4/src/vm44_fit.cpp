// ============================================================================
// vm44_fit.cpp - V4.3 WcsFitter 模块 (Phase E / IRM Step 4)
//
...
// ============================================================================
// vm44_fit - 主入口: 分层 Huber 稳健 LSQ 拟合
//
    ...
    Logger* logger)
{
    ...
    LOG_INFO("=== vm44_fit 开始 (n_pairs=" + std::to_string(n_pairs) +
             ", N_img=" + std::to_string((int)U.size()) +
             ", M=" + std::to_string((int)W.size()) + ") ===");

    // --- 边界检查 ---
    if (n_pairs < 2) {
        LOG_WARN("vm44_fit: n_pairs < 2, 无法拟合");
        return -1;
    }
    if (focal_length_mm <= 0 || pixel_size_um <= 0) {
        LOG_ERROR("vm44_fit: focal_length_mm 或 pixel_size_um <= 0");
        return -1;
    }
    ...
    if (skip_sip || M_clean < irm_sip_min_pairs) {
        ...
        LOG_INFO("vm44_fit 完成: CD=[" + std::to_string(output.cd.cd11) + "," +
