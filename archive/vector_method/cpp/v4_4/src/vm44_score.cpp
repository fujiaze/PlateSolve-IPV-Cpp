// ============================================================================
// vm44_score.cpp - V4.3 S_robust 稳健评分模块 (IRM Step 5)
//
...
// 注: 系数已反归一化 (与 vm44_fit Layer 2 一致), 输入应为原始像素坐标
    ...
    Logger* logger)
{
    ...
    if (N <= 0) {
        if (logger) logger->warn("vm44_compute_s_robust: 控制点为空");
        return -1;
    }
    if (s0 <= 0) {
        if (logger) logger->error("vm44_compute_s_robust: s0 <= 0");
        return -1;
    }

    if (logger) {
        logger->infof("vm44_compute_s_robust: N=%d, M0=%d, s0=%.4f\", sip_order=%d",
                      N, M0, s0, sip.order);
    }
    ...
    if (std::abs(cd_det) < 1e-20) {
        if (logger) logger->error("vm44_compute_s_robust: CD 行列式接近 0");
        return -1;
    }
    double cdi2[4] = {
    ...
    // -------------------------------------------------------------------
    // Step A: 计算偏移 (像素空间, 与 vm44_fit Layer 1 一致)
    //
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (mp.u < 0 || mp.u >= (int)U.size() ||
            ...
            if (logger) logger->warnf("vm44_compute_s_robust: 控制点 %d 索引越界", i);
            continue;
        }
        ...
        // SIP 修正 (像素空间, 在实际像素坐标处求值)
        // vm44_fit: rx_h = ddx - ssx ≈ SIP(ssx, ssy) → ddx - SIP ≈ ssx
        double du, dv;
    ...
    if (n_valid <= 0) {
        if (logger) logger->error("vm44_compute_s_robust: 无有效残差");
        return -1;
    }
    ...
    if (logger) {
        logger->infof("vm44_compute_s_robust: t_median=(%.4f, %.4f)px, "
