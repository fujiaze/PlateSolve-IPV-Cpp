// ============================================================================
// vm44_match.cpp - V4.3 VectorMatcher 模块 (Phase A+B)
//
// 职责: PROSAC 抽样 + θ 直方图投票 + 三级过滤 + Umeyama SVD 精修
// 4 模式并行 (OpenMP), 选择最优模式
//
// 从 V4.2 vm_core.cpp + vm_prosac.cpp 迁移, 合并为单一文件:
//   - namespace v42 → v43
//   - 接口改为内部 C++ 函数 v44::vm44_match (无 ctypes 边界, 无 JSON 序列化)
//   - 输入从裸 double 数组改为 std::vector<StarPoint>
//   - 输出从 C struct (cu/cw 指针) 改为 std::vector<MatchPair>
//   - 移除 __declspec(dllexport) 和 C 接口 vm_match/vm_free_result
//   - 保留全部核心算法逻辑 (PROSAC + θ 直方图 + 三级过滤 + Umeyama SVD + 4 模式并行)
//
// 依赖: Eigen3 (SVD), OpenMP (4 模式并行)
// ============================================================================
...
// ============================================================================
// PROSAC 优先采样 (从 vm_prosac.cpp 迁移)
// 基于 Chum 2005 综述: 按质量分降序, 优先从高质量星采样
// ============================================================================
...
static std::vector<StarQuality> compute_quality_score(
...
// PROSAC 增长函数 g(t) = n × (t/T_max)^(1/3)
static int prosac_pool_size(int t, int n, int T_max) {
    if (n <= 0) return 0;
    if (n == 1) return 1;
...
public:
    ...
    int last_pool_size() const { return last_pool_size_; }
    double quality_median() const { return quality_median_; }
...
// ============================================================================
// 线性扫描 NN (替代 nanoflann KDTree, 复杂度 O(N*M))
// ============================================================================
    ...
    Logger& logger)
{
    ...
    for (int iter = 0; iter < Kmax; ++iter) {
        ...
        double td = wrap180(theta * VM44_RADTODEG);
        double ct = std::cos(theta), st = std::sin(theta);
    ...
    logger.info("PhaseB OK: s=" + std::to_string(res.s) +
                " θ=" + std::to_string(res.theta * VM44_RADTODEG) + "°" +
...
// ============================================================================
// V4.4: 相对向量法 Phase A + Phase B
// 替代 record_and_filter, 用 vm44_relvec_match 的 θ_peak + 通过候选构建 records,
// 然后复用 Phase B 的三级过滤 + 1对1 + Umeyama SVD + 迭代精修
// ============================================================================
static PhaseABResult relvec_phase_ab(
    const double* U, int N, const double* Wf, int M,
    double s0, double s_min, double s_max,
    int min_inliers, double fov_diag_asec,
    const RelVecResult& relvec,
    Logger& logger)
{
    PhaseABResult res; res.s = 1; res.theta = 0; res.tx = 0; res.ty = 0;
    res.n_inliers = 0; res.rms = 1e30; res.peak_snr = relvec.theta_snr;
    res.n_samples = relvec.n_samples; res.success = false;
    res.theta_peak_deg = relvec.theta_peak_deg; res.best_n_range = 0;
    res.median_noise = 0; res.n_phasea_records = (int)relvec.passed_pairs.size();
    res.n_phaseb_corr = 0; res.prosac_quality_median = 0.0; res.prosac_pool_final = 0;

    if (!relvec.success || relvec.passed_pairs.size() < 2) {
        logger.warn("relvec_phase_ab: relvec 未成功或通过候选 <2");
        return res;
    }

    double tp_deg = relvec.theta_peak_deg;
    double s_peak = relvec.s_peak;
    double dx_peak = relvec.dx_peak;
    double dy_peak = relvec.dy_peak;
    // s 容差: 1% 相对容差 (真阳性 s 一致性 <0.5%, 假阳性偏差通常 >5%)
    double s_tol = std::max(0.01, std::abs(s_peak) * 0.01);
    // 注: dx/dy 不过滤 - 单点法下 4 种等价情况分散到 (dx,dy) 平面不同点
    //     3D 峰值可能选到非真值簇, 用 dx/dy 过滤会误删真值候选
    //     让 Phase B 的 θ 过滤 + Umeyama SVD + MAD 自己处理 dx/dy 歧义
    logger.info("RelVec PhaseA: θ=" + std::to_string(tp_deg) + "°" +
                " s=" + std::to_string(s_peak) +
                " dx=" + std::to_string(dx_peak) +
                " dy=" + std::to_string(dy_peak) +
                " SNR=" + std::to_string(relvec.theta_snr) + "x" +
                " passed=" + std::to_string(relvec.passed_pairs.size()) +
                " focused=" + std::to_string(relvec.n_focused) +
                " s_tol=" + std::to_string(s_tol) +
                " (dx/dy 不过滤, 让 RANSAC 处理歧义)");

    // 构建 records: 同时使用 (img_i, gaia_a) 和 (img_j, gaia_b) 两个映射
    // 只用 s 过滤 (θ 过滤在后面 Phase B 做, dx/dy 不过滤因歧义问题)
    std::vector<PairRecord> records;
    records.reserve(relvec.passed_pairs.size() * 2);
    int n_filtered_by_s = 0;
    for (const auto& p : relvec.passed_pairs) {
        // s 聚类过滤: s 必须接近峰值 (θ, dx, dy 过滤在 Phase B 做)
        if (std::abs(p.s_est - s_peak) > s_tol) { n_filtered_by_s++; continue; }

        PairRecord r1;
        r1.u_idx = p.img_i;
        r1.w_idx = p.gaia_a;
        r1.theta_deg = p.theta_rot_deg;
        r1.n_in_range_s = 100;  // 已通过第三星验证, 标记为高质量
        records.push_back(r1);

        PairRecord r2;
        r2.u_idx = p.img_j;
        r2.w_idx = p.gaia_b;
        r2.theta_deg = p.theta_rot_deg;
        r2.n_in_range_s = 100;
        records.push_back(r2);
    }
    logger.info("RelVec PhaseB: s过滤 θ=" + std::to_string(tp_deg) + "°" +
                " s=" + std::to_string(s_peak) +
                " 过滤掉 s错=" + std::to_string(n_filtered_by_s) +
                ", 保留 " + std::to_string(records.size() / 2) + " 对" +
                " (dx/dy 不过滤)");
    ...
    // 相对向量法候选质量高, 直接用 θ 带宽 4° 过滤 (无需三级放宽)
    nthr = 1; tband = 4.0;
    for (auto& r : records) {
        if (angle_diff_deg(r.theta_deg, tp_deg) > tband) continue;
        filt.push_back(r);
    ...
    if (filt.size() < 2) {
        logger.warn("RelVec PhaseB: <2 pairs after filter");
        return res;
    }
    ...
    res.cu = cu; res.cw = cw;
    logger.info("RelVec PhaseB: " + std::to_string(cu.size()) + " corr (filtered " +
                std::to_string(filt.size()) + " pairs)");
    if (cu.size() < 2) {
        logger.warn("RelVec PhaseB: <2 corr after 1to1");
        return res;
    }
    ...
    // Fallback 计算: 用 θ_peak + passed_pairs 的中位数 s/t 构造初值
    // 相对向量法定义: θ_rot = angle(Δw) - angle(Δu) = -θ_true
    // → θ_true = -θ_peak (Phase B umeyama 求 U=s·R(θ_true)·Wf+t 的 θ_true)
    // 当 SVD 无效 或 iterative_svd_refine 失败时使用
    auto compute_fallback = [&]() -> SimTransform {
        double fb_theta = -tp_deg * VM44_DEGTORAD;
        double ctf = std::cos(fb_theta), stf = std::sin(fb_theta);

        // 只用 (θ_peak ± 2°, s_peak ± s_tol) 内的 passed_pairs (2D 聚类双过滤, 更可能是真匹配)
        // 估计: s = |Δu|/|Δw|, t = U[img_i] - s·R(θ)·Wf[gaia_a]
        std::vector<double> s_estimates, tx_estimates, ty_estimates;
        s_estimates.reserve(relvec.passed_pairs.size());
        tx_estimates.reserve(relvec.passed_pairs.size());
        ty_estimates.reserve(relvec.passed_pairs.size());
        for (const auto& p : relvec.passed_pairs) {
            if (angle_diff_deg(p.theta_rot_deg, tp_deg) > 2.0) continue;
            if (std::abs(p.s_est - s_peak) > s_tol) continue;  // 2D 聚类 s 过滤

            // s 估计
            double dxu = U[p.img_i*2]   - U[p.img_j*2];
            double dyu = U[p.img_i*2+1] - U[p.img_j*2+1];
            double du  = std::sqrt(dxu*dxu + dyu*dyu);
            double dxw = Wf[p.gaia_a*2]   - Wf[p.gaia_b*2];
            double dyw = Wf[p.gaia_a*2+1] - Wf[p.gaia_b*2+1];
            double dw  = std::sqrt(dxw*dxw + dyw*dyw);
            if (dw < 1e-6) continue;
            double s_pair = du / dw;
            s_estimates.push_back(s_pair);

            // t 估计: t = U[img_i] - s·R(θ)·Wf[gaia_a]
            double wx = Wf[p.gaia_a*2], wy = Wf[p.gaia_a*2+1];
            double rwx = ctf*wx - stf*wy;
            double rwy = stf*wx + ctf*wy;
            tx_estimates.push_back(U[p.img_i*2]   - s_pair * rwx);
            ty_estimates.push_back(U[p.img_i*2+1] - s_pair * rwy);
        }

        double fb_s = 1.0, fb_tx = 0, fb_ty = 0;
        if (!s_estimates.empty()) {
            std::sort(s_estimates.begin(), s_estimates.end());
            fb_s = s_estimates[s_estimates.size() / 2];
        }
        if (!tx_estimates.empty()) {
            std::sort(tx_estimates.begin(), tx_estimates.end());
            std::sort(ty_estimates.begin(), ty_estimates.end());
            fb_tx = tx_estimates[tx_estimates.size() / 2];
            fb_ty = ty_estimates[ty_estimates.size() / 2];
        } else {
            // 无过滤后 pairs, 回退到质心对齐
            double ucx = 0, ucy = 0, wcx = 0, wcy = 0;
            for (int i = 0; i < N; ++i) { ucx += U[i*2]; ucy += U[i*2+1]; }
            for (int j = 0; j < M; ++j) { wcx += Wf[j*2]; wcy += Wf[j*2+1]; }
            ucx /= N; ucy /= N; wcx /= M; wcy /= M;
            fb_tx = ucx - fb_s * (ctf*wcx - stf*wcy);
            fb_ty = ucy - fb_s * (stf*wcx + ctf*wcy);
        }

        SimTransform fb;
        fb.s = fb_s; fb.theta = fb_theta;
        fb.tx = fb_tx; fb.ty = fb_ty; fb.valid = true;

        logger.info("RelVec PhaseB fallback: s=" + std::to_string(fb_s) +
                    " θ=" + std::to_string(fb_theta * VM44_RADTODEG) + "°" +
                    " tx=" + std::to_string(fb_tx) + " ty=" + std::to_string(fb_ty) +
                    " (filtered=" + std::to_string(s_estimates.size()) + " pairs)");
        return fb;
    };

    auto sim = umeyama(sp.data(), dp.data(), (int)cu.size());
    if (!sim.valid) {
        // SVD 无效(候选对共线或退化), fallback
        logger.warn("RelVec PhaseB: SVD invalid, fallback to θ_peak + filtered pairs median");
        sim = compute_fallback();
    }

    // 迭代 SVD 精修
    std::vector<double> Wt(M * 2);
    auto ref = iterative_svd_refine(U, N, Wf, M, sim.s, sim.theta, sim.tx, sim.ty, s0, 10);
    if (ref.success && ref.n_inliers >= 3) {
        res.s = ref.s; res.theta = ref.theta;
        res.tx = ref.tx; res.ty = ref.ty;
        res.n_inliers = ref.n_inliers; res.rms = ref.rms;
    } else {
        // iterative_svd_refine 失败 (n_inliers<3): Umeyama 被 corr 中假阳性污染
        // 用 fallback (θ_peak + median s/t) 重试
        logger.warn("RelVec PhaseB: iterative_svd_refine failed (n_inliers=" +
                    std::to_string(ref.n_inliers) + "), retry with fallback");
        sim = compute_fallback();
        auto ref2 = iterative_svd_refine(U, N, Wf, M, sim.s, sim.theta, sim.tx, sim.ty, s0, 10);
        if (ref2.success && ref2.n_inliers >= 3) {
            res.s = ref2.s; res.theta = ref2.theta;
            res.tx = ref2.tx; res.ty = ref2.ty;
            res.n_inliers = ref2.n_inliers; res.rms = ref2.rms;
        } else {
            // fallback 也失败, 用 fallback sim 直接计算 inliers
            apply_similarity(Wf, M, sim.s, sim.theta, sim.tx, sim.ty, Wt.data());
            auto inl = count_inliers_1to1(U, N, Wt.data(), M, 1.0 * s0);
            res.s = sim.s; res.theta = sim.theta;
            res.tx = sim.tx; res.ty = sim.ty;
    ...
    res.success = true;
    res.n_phaseb_corr = (int)cu.size();
    logger.info("RelVec PhaseB OK: s=" + std::to_string(res.s) +
                " θ=" + std::to_string(res.theta * VM44_RADTODEG) + "°" +
                " n=" + std::to_string(res.n_inliers) +
                " rms=" + std::to_string(res.rms) +
                " corr=" + std::to_string(cu.size()));
...
static ModeRes solve_single_mode(
    ...
    const std::vector<StarPoint>& U_stars,
    const std::vector<StarPoint>& W_stars,
    ...
    Logger& logger)
{
    ...
    // V4.4: 相对向量法完全替代单θ Phase A
    // 1. 构造翻转后的 Wf_starpoint 数组 (供 vm44_relvec_match 使用)
    std::vector<StarPoint> Wf_stars(M);
    for (int j = 0; j < M; ++j) {
        Wf_stars[j].x = Wf[j*2];
        Wf_stars[j].y = Wf[j*2+1];
        Wf_stars[j].flux = W_stars[j].flux;
        Wf_stars[j].saturated = W_stars[j].saturated;
    }

    // 2. 调用 vm44_relvec_match (Phase A)
    RelVecResult relvec;
    int rv_ret = vm44_relvec_match(U_stars, Wf_stars, s0, params, relvec, &logger);
    if (rv_ret != 0 || !relvec.success) {
        logger.warn("mode" + std::to_string(mode) + " 相对向量法 Phase A 失败 (SNR=" +
                    std::to_string(relvec.theta_snr) + ")");
        mr.peak_snr = relvec.theta_snr;
        mr.n_samples = relvec.n_samples;
        return mr;
    }
    logger.info("mode" + std::to_string(mode) + " 相对向量法: θ=" +
                std::to_string(relvec.theta_peak_deg) + "° SNR=" +
                std::to_string(relvec.theta_snr) + "x passed=" +
                std::to_string(relvec.n_passed));

    // 3. Phase B: 用 θ_peak + 通过候选构建 records, 复用 Phase B 逻辑
    mr.ab = relvec_phase_ab(U, N, Wf.data(), M, s0, s_min, s_max,
                             min_inliers, fov_diag_asec, relvec, logger);
    mr.peak_snr = mr.ab.peak_snr; mr.n_samples = mr.ab.n_samples;
    if (!mr.ab.success) {
        logger.warn("mode" + std::to_string(mode) + " Phase B 失败");
        return mr;
    }
...
int vm44_match(
    ...
    Logger* logger)
{
    // 默认 logger (logger 为 null 时用默认, 仅输出 stderr)
    Logger default_logger;
    Logger& log = logger ? *logger : default_logger;

    log.info("=== vm44_match 开始 ===");
    log.info("N_img=" + std::to_string(U_stars.size()) +
             " M=" + std::to_string(W_stars.size()) +
             " s0=" + std::to_string(s0) +
             " s_min=" + std::to_string(params.s_min) +
             " s_max=" + std::to_string(params.s_max) +
             " n_modes=" + std::to_string(params.n_modes));

    int N = (int)U_stars.size();
    int M = (int)W_stars.size();

    if (N < 2 || M < 2) {
        log.error("vm44_match: N 或 M 过少 (N=" + std::to_string(N) +
                  " M=" + std::to_string(M) + ")");
        output.success = false;
        return -1;
    }

    // 将 StarPoint 转为裸 double 数组 (保持 V4.2 算法逻辑不变)
    std::vector<double> U(N * 2), W(M * 2);
    for (int i = 0; i < N; ++i) {
        U[i*2]   = U_stars[i].x;
        U[i*2+1] = U_stars[i].y;
    }
    ...
    // 构建饱和标志数组 (从 StarPoint.saturated, 供 PROSAC 质量分使用)
    std::vector<int> is_sat(N, 0);
    for (int i = 0; i < N; ++i) {
        is_sat[i] = U_stars[i].saturated ? 1 : 0;
    }

    // FOV 对角线估算 (与 V4.2 一致: max_t = fov_diag*0.6 ≈ s0×5000 经验值)
    double fov_diag_asec = 0.0;
    if (s0 > 0) {
        fov_diag_asec = std::max(1000.0, 5000.0 * s0);
    }

    int n_modes = std::max(1, std::min(params.n_modes, 4));
    log.info("开始 " + std::to_string(n_modes) + " 模式并行求解");

    // 4 模式并行 (OpenMP)
    std::vector<ModeRes> mres(n_modes);
    #pragma omp parallel for schedule(static)
    for (int m = 0; m < n_modes; ++m) {
        mres[m] = solve_single_mode(U.data(), N, W.data(), M,
                     U_stars, W_stars,
                     m, s0,
                     ...
                     nullptr, is_sat.data(),  // SNR 用 nullptr (V4.2 fallback 到 sparsity 倒数)
    ...
    for (int m = 0; m < n_modes; ++m) {
        ...
        log.info("mode" + std::to_string(m) + " 评分: n_inliers=" +
    ...
    output.transform.s     = best.s;
    output.transform.theta = best.theta;
    output.transform.tx    = best.tx;
    output.transform.ty    = best.ty;
    output.transform.valid = true;
    output.rms             = best.rms;
    output.theta_snr       = best.ab.peak_snr;
    output.theta_peak_deg  = best.ab.theta_peak_deg;
    output.best_n_range    = best.ab.best_n_range;
    output.n_phasea_records = best.ab.n_phasea_records;
    output.prosac_quality_median = best.ab.prosac_quality_median;
    output.prosac_pool_final     = best.ab.prosac_pool_final;
    output.best_mode       = best_mode;
    output.success         = true;

    // 转换 cu/cw 索引为 MatchPair 数组
    output.pairs.clear();
    output.pairs.reserve(best.ab.cu.size());
    for (size_t k = 0; k < best.ab.cu.size(); ++k) {
        output.pairs.push_back({best.ab.cu[k], best.ab.cw[k]});
    }

    log.info("=== vm44_match 完成 === mode=" + std::to_string(best_mode) +
             " s=" + std::to_string(best.s) +
             " θ=" + std::to_string(best.theta * VM44_RADTODEG) + "°" +
             " n_inliers=" + std::to_string(best.n_inliers) +
             " rms=" + std::to_string(best.rms) + "\"" +
             " pairs=" + std::to_string((int)output.pairs.size()));
