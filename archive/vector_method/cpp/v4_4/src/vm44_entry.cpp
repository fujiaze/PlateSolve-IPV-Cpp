// ============================================================================
// vm44_entry.cpp - V4.4 单一入口 vm44_solve() 实现
//
// 对外暴露的 C 接口, 内部串联:
...
//   2. Phase 0: vm44_select(image_path, ...) → StarSelection (U + W + 元数据)
//   3. Phase A+B: vm44_match(U, W, s0, ...) → VectorMatchResult (transform + C₀ + best_mode)
//   4. 应用 flip (best_mode) 到 W
//   5. 从 SimTransform 推导 CD₀
//   6. IRM 闭环: vm44_irm_refine(C0, CD0, ...) → 最终 CD + SIP + control_points
//   7. 填充 VM44SolveResult
//
// Task 9 实现
// ============================================================================
    ...
    v44::VM44SolveResult* result)
{
    ...
    std::string log_path;
    if (p.log_dir) {
        log_path = std::string(p.log_dir) + "/vm44_solve.log";
        logger.init(log_path);
    }
    logger.infof("=== vm44_solve 开始 (image=%s, RA=%.6f, Dec=%.6f) ===",
                 image_path ? image_path : "(null)", ra, dec);

    // --- 边界检查 ---
    if (!image_path) {
        std::strncpy(result->error_msg, "image_path 为空", sizeof(result->error_msg) - 1);
        logger.error("vm44_solve: image_path 为空");
        return -1;
    }
    ...
    v44::VectorMatchResult vm_result;
    rc = v44::vm44_match(selection.U, selection.W, s0, p, vm_result, &logger);
    if (rc != 0 || !vm_result.success) {
        std::strncpy(result->error_msg, "Phase A+B VectorMatcher 失败",
                     sizeof(result->error_msg) - 1);
        logger.error("vm44_solve: Phase A+B 失败");
        return -1;
    }
    ...
    // --- 应用 flip 到 W ---
    std::vector<v44::StarPoint> W_flipped = selection.W;
    v44::apply_flip_to_w(W_flipped, vm_result.best_mode);
    if (vm_result.best_mode != 0) {
        logger.infof("应用 flip mode=%d 到 W", vm_result.best_mode);
    }

    // --- 从 SimTransform 推导 CD₀ ---
    v44::CDMatrix CD0 = v44::sim_to_cd(vm_result.transform, s0);
    ...
    std::vector<v44::MatchPair> final_control_points;
    v44::SRobustResult final_s_robust;
    int n_iters = 0;
    bool converged = false;
    double final_bayes_lnK = 0.0;
    double final_triangle_pass_ratio = 0.0;

    rc = v44::vm44_irm_refine(
        vm_result.pairs, CD0,
    ...
    if (rc != 0) {
        std::strncpy(result->error_msg, "IRM 闭环失败",
                     sizeof(result->error_msg) - 1);
        logger.error("vm44_solve: IRM 闭环失败");
        return -1;
    }
    ...
    result->success = true;
    logger.infof("=== vm44_solve 完成: N=%d, S_robust=%.4f\", RMS=%.4f px ===",
...
VM44_API void vm44_get_default_params(v44::VM44SolveParams* params)
{
    ...
    params->relvec_n_samples = 5000;           // 最大采样上限 (自适应停止可能提前结束)
    ...
    params->relvec_n_third_stars = 10;         // 第三星验证颗数 (投票数无上限, 通过m颗投m票)
    // 自适应采样停止参数
