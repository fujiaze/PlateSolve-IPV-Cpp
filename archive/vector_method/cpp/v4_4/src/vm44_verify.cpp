// ============================================================================
// vm44_verify.cpp - V4.3 PairVerifier 模块 (Phase D / IRM Step 3)
//
...
//
// 接口:
//   int vm44_verify(candidates, U, W, s0, fov_diag_deg, params, output, logger)
//
// 算法流程:
    ...
    Logger* logger)
{
    ...
    if (n_pairs < 3) {
        ...
        if (logger) logger->warnf("vm44_verify mad: 对数 < 3, 跳过清洗 (n=%d)", n_pairs);
        return result;
    }
    ...
    if (!sim.valid) {
        ...
        if (logger) logger->warn("vm44_verify mad: 初始 Umeyama 失败, 使用恒等变换");
    }
    ...
    std::vector<bool> keep(n_pairs, true);
    {
        ...
        if (logger) logger->debugf("vm44_verify mad 预过滤检查: init_med=%.3f min_thresh=%.3f valid=%d",
        ...
        if (init_med > min_thresh) {
            ...
            if (n_pre_removed > 0 && n_keep >= 3) {
                ...
                if (sim2.valid) {
                    sim = sim2;
                    apply_similarity(W, sim.s, sim.theta, sim.tx, sim.ty, Wt);
                    if (logger) logger->debugf("vm44_verify mad 预过滤触发: init_med=%.3f pre_thresh=%.3f removed=%d keep=%d",
    ...
    for (; iter < max_iters; ++iter) {
        ...
        if (logger) logger->debugf("vm44_verify mad 轮%d: thresh=%.3f removed=%d keep=%d",
    ...
    if (logger) logger->infof("vm44_verify mad: %d 轮, 清洗后 %d 对 (剔除 %d), RMS=%.3f\"",
    ...
    Logger* logger)
{
    ...
    if (n < 4) {
        ...
        r.n_inliers = n;
        r.success = true;
        if (logger) logger->warnf("vm44_verify ransac: n=%d < 4, 跳过", n);
        return r;
    }
    ...
    if (logger) logger->infof("vm44_verify ransac: best_inliers=%d/%d (min=%d) success=%d",
    ...
    Logger* logger)
{
    ...
    if (matched_pairs.empty()) {
        if (logger) logger->warn("vm44_verify bayes: 匹配对为空, 返回拒绝");
        return result;
    }
    ...
    if (sigma <= 0.0) {
        if (logger) logger->warn("vm44_verify bayes: sigma 无效, 返回拒绝");
        return result;
    }
    result.sigma = sigma;
    ...
    if (fov_diag_deg <= 0.0) {
        if (logger) logger->warn("vm44_verify bayes: fov_diag_deg 无效, 返回拒绝");
        return result;
    }
    ...
    if (logger) logger->infof("vm44_verify bayes: n=%d σ=%.3f RMS=%.3f A_fov=%.0f lnK=%.3f decision=%d",
    ...
    Logger* logger)
{
    ...
    int n = (int)matched_pairs.size();
    if (n < 3) {
        if (logger) logger->warnf("vm44_verify triangle: n=%d < 3, 跳过", n);
        return r;
    }
    ...
    if (logger) logger->infof("vm44_verify triangle: n=%d total=%d passed=%d ratio=%.3f accepted=%d",
    ...
    Logger* logger)
{
    ...
    if (logger) logger->infof("=== vm44_verify 开始 (n_candidates=%d, N_img=%d, M=%d, fov=%.3f°) ===",
                               (int)candidates.size(), (int)U.size(), (int)W.size(), fov_diag_deg);

    // --- 边界检查 ---
    if (candidates.empty()) {
        if (logger) logger->warn("vm44_verify: candidates 为空");
        return -1;
    }
    ...
    if (output.n_clean < 3) {
        if (logger) logger->warnf("vm44_verify: 清洗后对数 < 3, 跳过验证 (n_clean=%d)",
                                   output.n_clean);
        output.validated = false;
        output.success = true;
        return 0;
    ...
    if (logger) logger->infof("=== vm44_verify 完成: validated=%d (bayes=%d tri=%d ransac=%d) ===",
