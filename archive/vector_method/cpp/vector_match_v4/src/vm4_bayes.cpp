...
// 默认阈值（与 vm4_api.h 中 VM4SolveParams 字段一致）
    ...
    double lnK_accept, double lnK_weak)
{
    ...
    // --- 边界检查 ---
    if (residuals.empty()) {
        fprintf(stderr, "[vm4_bayes] 警告: 残差数组为空，返回拒绝\n");
        return result;
    }
    if (sigma <= 0.0) {
        fprintf(stderr, "[vm4_bayes] 警告: sigma=%.4f 无效（需>0），返回拒绝\n", sigma);
        return result;
    }
    if (A_fov_sqdeg <= 0.0) {
        fprintf(stderr, "[vm4_bayes] 警告: A_fov=%.4f 平方度无效（需>0），返回拒绝\n", A_fov_sqdeg);
        return result;
    }
    ...
    // --- 日志输出 ---
    fprintf(stderr, "[vm4_bayes] n_match=%d sigma=%.3f\" RMS=%.3f\" "
    ...
    double lnK_accept, double lnK_weak)
{
    ...
    fprintf(stderr, "[vm4_bayes] verify_match_bayes: %zu 对匹配，sigma=%.3f\"，A_fov=%.2f sqdeg\n",
