...
#ifdef _WIN32
#define VM4_API __declspec(dllexport)
#else
#define VM4_API __attribute__((visibility("default")))
#endif

struct VM4SolveParams {
    // === V3.5 继承字段（保留原样）===
    double s0;
    double s_min;
    double s_max;
    ...
    int    m_lim_max_iter;    // 极限星等迭代最大次数，默认8
...
VM4_API int vm4_solve(
    const double* U,
    int N_img,
    const double* W,
    int M,
    const VM4SolveParams* params,
    VM4SolveResult* result
);

VM4_API int vm4_count_inliers(
...
VM4_API int vm4_write_wcs_file(
    const VM4SolveResult* result,
    const char* path
);
