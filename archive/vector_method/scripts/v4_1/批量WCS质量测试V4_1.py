"""V4.1 批量WCS质量测试 — 随机抽取testdata中的帧进行WCS求解+重投影质量评估

功能:
    随机抽取testdata/lights下的FITS帧(默认50帧) → V4.1求解WCS
    → 标准WCS-SIP重投影Gaia亮星 → 统计投影命中率
    → 结果保存JSON+CSV供分析

用途:
    评估V4.1在不同目标/滤镜/曝光下的WCS求解成功率和精度

用法:
    python 批量WCS质量测试V4_1.py [帧数] [随机种子]
    默认: 50帧, 种子=42(可复现)

V4.1 参数整定:
    - img_n_target=50 (图像侧目标星数, 不对称选星)
    - gaia_density_ratio=1.5 (Gaia面密度/图像面密度)
    - gaia_query_radius_factor=0.55 (Gaia查询半径因子, 即0.55×FOV对角线)
    - 相对V4.0改进: 不对称选星 + 自适应密度匹配查询半径
"""
...
logger = logging.getLogger("批量WCS质量测试V4_1")
...
def solve_single_frame(fits_path, solver, output_dir, n_bright=1000):
    """对单帧FITS进行V4.1 WCS求解+重投影质量评估

    Args:
        fits_path: FITS文件路径
        solver: VectorMatchV4_1Cpp 实例(复用)
...
def main():
    ...
    # 输出目录 (V4.1专属)
    output_dir = os.path.join(PROJECT_ROOT, "lib", "plate_solve", "logs", "v4_1", "batch_test")
    ...
    # 复用solver实例 (V4.1)
    solver = VectorMatchV4_1Cpp(os.path.join(PROJECT_ROOT, "GaiaDR3"), db_type=1)
