"""V4.4 全量测试 - 遍历 testdata 下所有 FITS 文件

功能:
    递归收集 testdata/ 下所有 FITS 文件 (.fts/.fit/.fits), 全部用 V4.4 求解
    V4.4 用相对向量法 (DMPDV, 加权投票) 完全替代单θ Phase A
    记录 success/RMS/matched/耗时/S_robust/n_iters/irm_converged/sip_order/lnK/theta_snr
    按目标/滤镜分组统计, 输出失败帧列表

输出:
    - lib/plate_solve/logs/v4_4/full_test/full_test_all.json
    - lib/plate_solve/logs/v4_4/full_test/summary.csv
    - lib/plate_solve/logs/v4_4/full_test/summary.txt

用法:
    python 全量测试V4_4.py
    python 全量测试V4_4.py --no-resume    # 忽略断点续跑, 重新测试
...
# MSYS2 MinGW DLL 路径 (V4.4 DLL 依赖 libwinpthread-1.dll / libgcc_s_seh / libstdc++ / libgomp)
...
logger = logging.getLogger("V4.4全量测试")
...
_OUTPUT_DIR = os.path.join(
    PROJECT_ROOT, "lib", "plate_solve", "logs", "v4_4", "full_test")
...
def main():
    parser = argparse.ArgumentParser(description="V4.4 全量测试")
    parser.add_argument("--no-resume", action="store_true",
                        help="忽略断点续跑, 重新测试所有帧")
    args = parser.parse_args()

    print(f"=== V4.4 全量测试 (相对向量法 加权投票) ===")
    ...
    # 创建 V4.4 Solver
    print("创建 V4.4 V44Solver (相对向量法 + IRM 闭环)...")
    ...
    for i, fits_path in enumerate(all_fits, 1):
        ...
        info = solve_single_frame_v44(fits_path, solver)
        results.append(info)
        n_new_done += 1
        ...
        if info["status"] == "success":
            print(f"  → 成功: mode={info['flip_mode']} n={info['matched_count']} "
                  ...
                  f"SNR={info['theta_snr']} 耗时={info['solve_time_s']}s")
        else:
            print(f"  → {info['status']}")
...
def _save_csv(results):
    ...
    csv_fields = [
        ...
        "sip_order", "bayes_lnK", "triangle_pass_ratio", "theta_snr", "theta_peak_deg",
...
def _build_summary(results):
    ...
    lines.append("=" * 70)
    lines.append("  V4.4 全量测试汇总 (相对向量法 加权投票)")
    lines.append("=" * 70)
    ...
    if n_success > 0:
        ...
        snr_list = [r.get("theta_snr", 0) for r in succ]
        ...
        lines.append(f"  θ SNR: 中位={np.median(snr_list):.1f} 均值={np.mean(snr_list):.1f} "
