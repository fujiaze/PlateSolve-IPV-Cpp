"""
向量匹配 V2 鲁棒性测试 - 全testdata扫描

自动扫描testdata下所有 .fit/.fits/.fts 文件
输出CSV记录: 文件名, 望远镜, 滤镜, 曝光, 检测耗时, 解析耗时, 饱和星数, 星点数,
            投影误差RMS, 焦距, 像元尺寸, 比例尺, 旋转角, 翻转模式, 匹配数,
            中心RA/Dec, 结果, 失败原因
"""
...
class RobustnessTester:
    ...
    def run(self, frame_paths: List[str]) -> List[TestResult]:
        ...
        logger.info("鲁棒性测试启动: %d帧", n_frames)
        ...
        logger.info("鲁棒性测试完成: %d/%d成功 (%.1f%%), 总耗时%.1fs",
...
def print_report(results: List[TestResult], t_total: float):
    ...
    print(f"失败: {n-fail.__len__()} ({len(fail)/n*100:.1f}%)")
    print(f"总耗时: {t_total:.1f}s")

    # 按望远镜统计
    print(f"\n按望远镜统计:")
    print(f"  {'望远镜':<10} {'总数':>5} {'成功':>5} {'成功率':>7} {'中位RMS':>8} {'中位匹配':>8}")
    for tel in sorted(set(r.telescope for r in results)):
        ...
        print(f"  {tel:<10} {len(tr):>5} {len(tok):>5} "
              f"{len(tok)/len(tr)*100:>6.1f}% {med_rms:>7.3f}px {med_m:>7.0f}")
...
def main():
    ...
    csv_path = os.path.join(PROJECT_ROOT, "robustness_test_results.csv")
    ...
    tester = RobustnessTester(gaia_dir, db_type=0, on_result=on_result)
