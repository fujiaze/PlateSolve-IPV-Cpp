...
"""T4帧: θ+s联合精搜索验证
阶段1: 1点法预热, 找θ粗峰值
阶段2: θ精搜索(步长0.2°), 对每个θ用s粗搜索(步长0.01), 抽样(tx,ty)
阶段3: 在最佳(θ,s)附近精搜索

目标: 验证θ精搜索能否解决T4帧1点法失效问题
"""
...
for mode in [2]:
    ...
    theta_coarse = bin_centers[peak_idx]

    t1 = time.time()
    print(f"阶段1: θ_coarse={theta_coarse:.1f}° SNR={snr:.1f}x ({t1-t0:.2f}s)")

    # ── 阶段2: θ精搜索 + s粗搜索 ──
    # θ范围: 粗峰值±3°, 步长0.2°
    # s范围: [0.94, 1.06], 步长0.02
    # 对每个(θ,s), 抽样5000次搜索(tx,ty)

    theta_range = np.arange(theta_coarse - 3.0, theta_coarse + 3.1, 0.2)
    ...
    for theta_deg in theta_range:
        ...
        for s_try in s_range:
            ...
            if max_n > best_n:
                ...
                if max_n >= 20:
                    print(f"  命中: θ={theta_deg:.1f}° s={s_try:.2f} n={max_n} tx={best_tx:.1f} ty={best_ty:.1f}")

    t2 = time.time()
    if best_params:
        theta_best, s_best, tx_best, ty_best = best_params
        print(f"\n阶段2: θ={theta_best:.1f}° s={s_best:.2f} n={best_n} tx={tx_best:.1f} ty={ty_best:.1f} ({t2-t1:.1f}s)")
        print(f"V2参考: θ={V2_THETA}° s={V2_S} tx={V2_TX} ty={V2_TY}")

        # ── 阶段3: (θ,s)精搜索 ──
        theta_fine = np.arange(theta_best - 0.3, theta_best + 0.31, 0.1)
        s_fine = np.arange(max(0.90, s_best - 0.03), min(1.10, s_best + 0.031), 0.002)
        ...
        if best_params_fine:
            ...
            print(f"\n阶段3: θ={theta_f:.1f}° s={s_f:.3f} n={best_n_fine} tx={tx_f:.1f} ty={ty_f:.1f} ({t3-t2:.1f}s)")
