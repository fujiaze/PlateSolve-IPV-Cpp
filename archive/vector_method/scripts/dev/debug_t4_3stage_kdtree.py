...
"""T4帧: 三阶段1点法 - 高效版
阶段1: 1点法预热, 找θ峰值
阶段2: 固定θ, s粗搜索, 用KDTree加速内点计数
阶段3: 在最佳s附近精搜索

关键优化: 固定(s,θ)后, 遍历所有(u_i,w_j)对计算(tx,ty), 用KDTree快速统计内点数
这样不需要随机抽样, 直接穷举所有可能的(tx,ty)
"""
...
for mode in [2]:
    ...
    for s_try in s_coarse_values:
        ...
        for i in range(N_img):
            for j in range(M):
                ...
                n = sum(1 for p in pairs if len(p) > 0)
                if n > max_n:
                    max_n = n
        s_coarse_scores.append(max_n)
        print(f"  s={s_try:.2f}: max_n={max_n}")
    ...
    t2 = time.time()
    print(f"\n阶段2: s_best={s_coarse_best:.2f} n={n_coarse_best} ({t2-t1:.2f}s)")
    ...
    if max_n_fine > 0:
        ...
        print(f"\n阶段3: s={best_s_fine:.3f} θ={theta_peak:.1f}° tx={best_tx_fine:.1f} ty={best_ty_fine:.1f}")
