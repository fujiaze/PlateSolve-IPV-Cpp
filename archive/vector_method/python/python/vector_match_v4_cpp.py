...
logger = logging.getLogger("vector_match_v4_cpp")
...
class VM4SolveParamsC(ctypes.Structure):
    _pack_ = 8
    _fields_ = [
        # === V3.5 继承字段 ===
...
class VM4DebugInfoC(ctypes.Structure):
    _pack_ = 8
    _fields_ = [
        # === V3.5 继承字段 ===
...
class VM4SolveResultC(ctypes.Structure):
    _pack_ = 8
    _fields_ = [
...
def _find_dll() -> str:
    """查找 V4.0 DLL 路径"""
    p = os.path.join(_PROJECT_ROOT, "lib", "plate_solve", "cpp", "vector_match_v4", "vector_match_v4.dll")
    if os.path.exists(p):
        return p
...
def _find_log_dir() -> str:
    """查找日志目录（不存在则创建）"""
    log_dir = os.path.join(_PROJECT_ROOT, "lib", "plate_solve", "logs", "v4")
...
def _make_log_path() -> str:
    ...
    return os.path.join(log_dir, f"v4_{ts}.log")
...
def density_match_query(gaia_client: GaiaClientPy,
    ...
    sys.stderr.write(
        f"[vm4] Phase 0 密度匹配: n_img={n_img_bright} rho_img={rho_img:.3f}/deg² "
    ...
    for it in range(m_lim_max_iter):
        ...
        n_stars = len(ra_t)
        sys.stderr.write(
            f"[vm4] Phase 0 iter#{it}: mag={mag_query:.2f} → {n_stars} stars "
        ...
        if n_stars < n_target_low:
            ...
            sys.stderr.write(
                f"[vm4] Phase 0 收敛: mag={mag_query:.2f} n_gaia={n_stars} "
...
class VectorMatchV4Cpp:
    """V4.0 抽样投票向量法 Python 封装

    用法:
        with VectorMatchV4Cpp(gaia_data_dir, db_type=1) as solver:
    ...
    def __init__(self, gaia_data_dir: str, db_type: int = 1):
        ...
        self._dll.vm4_solve.argtypes = [
            ctypes.POINTER(ctypes.c_double), ctypes.c_int,
            ctypes.POINTER(ctypes.c_double), ctypes.c_int,
            ctypes.POINTER(VM4SolveParamsC), ctypes.POINTER(VM4SolveResultC),
        ]
        self._dll.vm4_solve.restype = ctypes.c_int
        self._closed = False
    ...
    def solve(self, img_x, img_y, img_flux, img_saturated,
        ...
        Args:
            ...
            k_match, m_lim_step, m_lim_max_iter, density_tolerance: 密度匹配参数
            ...
            n_img_total: 图像星点总数(饱和全选+非饱和按flux降序补足, 默认250=V4.1扫描最优值)
        ...
        # ====================================================================
        # 图像星点选取: N=n_img_total — 取全部饱和星 + flux 最高的非饱和星补足
        # 饱和星应选尽选(星点多→投票数高→只会增大计算量,不会导致解析失败)
        # n_img_total 默认 250(V4.0 兼容), V4.1 将由参数扫描实验整定最优值
        # ====================================================================
        ...
        sys.stderr.write(
            f"[vm4] Phase 0 完成: m_lim={dm['m_lim_final']:.2f} n_gaia={dm['n_gaia_final']} "
            f"ρ_img={dm['rho_img']:.3f} ρ_target={dm['rho_target']:.3f} "
            f"iter={dm['m_lim_iterations']}\n"
        )
        ...
        sys.stderr.write(f"[vm4] FOV 内 Gaia 星: {len(fov_idx)}/{len(cat_ra_all)}\n")
        ...
        if wcs_json_path is None:
            import tempfile
            fd, wcs_json_path = tempfile.mkstemp(suffix='.json', prefix='vm4_wcs_')
            os.close(fd)
        ...
        sys.stderr.write(f"[vm4] 日志文件: {log_file_path}\n")

        # ====================================================================
        # 构造 VM4SolveParamsC 并调用 vm4_solve
        # ====================================================================
        params = VM4SolveParamsC()
        # V3.5 继承字段
        params.s0 = s0
        params.s_min = 0.9
        params.s_max = 1.1
        ...
        if ret != 0 or result.success == 0:
            logger.warning("vm4_solve fail: ret=%d succ=%d", ret, result.success)
            # 即使失败也输出调试信息便于分析
            if result.debug.bayes_lnK != 0 or result.debug.triangle_total != 0:
                sys.stderr.write(
                    f"[vm4] 失败但已有部分结果: lnK={result.debug.bayes_lnK:.2f} "
        ...
        r.m_lim_iterations = dm['m_lim_iterations']
        ...
        sys.stderr.write(
            f"[vm4] 调试: lnK={r.bayes_lnK:.2f} tri_ratio={r.triangle_pass_ratio:.3f} "
