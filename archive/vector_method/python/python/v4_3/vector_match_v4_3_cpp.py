"""
vector_match_v4_3_cpp.py - V4.3 单一 DLL ctypes 封装

功能:
    V4.3 统一管线 ctypes 封装, 将 V4.2 的 5 个独立 DLL 调用合并为 1 次 vm43_solve() 调用
    内部串联: StarSelector → VectorMatcher → IRM 闭环 (Expand ↔ Verify ↔ Fit)

用途:
    Python 端调用 V4.3 求解器, 输入 FITS 路径 + 中心指向 + 焦距/像元,
    输出标准 WCS 参数 (CD/CRVAL/CRPIX/SIP) + IRM 调试信息 (S_robust/n_inliers/n_iters)

依赖:
    - vector_match_v4_3.dll (由 lib/plate_solve/cpp/v4_3/Makefile 编译)
    - gaia_client.dll (经 GaiaClientPy 封装, 可注入)
    - star_detector.dll (经 StarDetector 封装, 可注入)
    - numpy, ctypes

V4.3 vs V4.2 改进:
    1. 单一 DLL: 消除 5 次 ctypes 往返 + 5 次 JSON 序列化 + 10 次数组边界拷贝
    2. IRM 闭环: 模型改善 → 匹配更准 → 控制点增多 → 模型更好 (借鉴 ICP)
    3. S_robust 评分: 残差跳变检测外点, 替代 RMS
    4. 马氏距离自适应匹配: 容差随模型精度收紧
    5. 局部几何一致性过滤: K=8 邻星角距一致性 (Heyl 2013)
    6. Huber 稳健 LSQ: 大残差截断保护
"""
...
logger = logging.getLogger("v4_3_solver")
...
class VM43SolveParams(ctypes.Structure):
    """V4.3 求解参数 (对应 C 结构体 VM43SolveParams)"""
    _pack_ = 8
    _fields_ = [
        # 基础参数
...
class _VM43SolveResult(ctypes.Structure):
    """V4.3 求解结果 (对应 C 结构体 VM43SolveResult, 内部使用)"""
    _pack_ = 8
    _fields_ = [
...
_DEFAULT_DLL_PATH = os.path.join(
    _PROJECT_ROOT, "lib", "plate_solve", "cpp", "v4_3", "vector_match_v4_3.dll"
)
...
class V43Solver:
    ...
    将 V4.2 的 5 个独立 DLL 调用合并为 1 次 vm43_solve() 调用
    ...
    def __init__(self,
        ...
        Args:
            dll_path: vector_match_v4_3.dll 路径
                     None 时使用默认路径 lib/plate_solve/cpp/v4_3/vector_match_v4_3.dll
        ...
        if not os.path.exists(dll_path):
            raise FileNotFoundError(f"V4.3 DLL 未找到: {dll_path}")
        ...
        if gaia_client is not None:
            ...
            logger.info("V4.3 内部创建 GaiaClientPy (GaiaDR3)")
        ...
        if star_detector is not None:
            ...
            try:
                ...
                logger.info("V4.3 内部创建 StarDetector (fitRadius=0 自动)")
        ...
        self._closed = False
        logger.info("V43Solver 初始化完成 (dll=%s)", os.path.basename(dll_path))
    ...
    def _setup_argtypes(self):
        ...
        # vm43_solve(image_path, ra, dec, focal_length_mm, pixel_size_um,
        #            params*, result*) -> int
        dll.vm43_solve.argtypes = [
            ctypes.c_char_p,              # image_path
            ctypes.c_double,              # ra
            ctypes.c_double,              # dec
            ctypes.c_double,              # focal_length_mm
            ctypes.c_double,              # pixel_size_um
            ctypes.POINTER(VM43SolveParams),  # params (可为 NULL)
            ctypes.POINTER(_VM43SolveResult),  # result
        ]
        dll.vm43_solve.restype = ctypes.c_int

        # vm43_set_gaia_client(handle) -> void
        dll.vm43_set_gaia_client.argtypes = [ctypes.c_void_p]
        dll.vm43_set_gaia_client.restype = None

        # vm43_set_star_detector(handle) -> void
        dll.vm43_set_star_detector.argtypes = [ctypes.c_void_p]
        dll.vm43_set_star_detector.restype = None

        # vm43_free_result(result*) -> void
        dll.vm43_free_result.argtypes = [ctypes.POINTER(_VM43SolveResult)]
        dll.vm43_free_result.restype = None

        # vm43_get_default_params(params*) -> void
    ...
    def _inject_handles(self):
        """从 GaiaClientPy / StarDetector 提取内部句柄, 注入 V4.3 DLL"""
        ...
        self._dll.vm43_set_gaia_client(ctypes.c_void_p(gaia_handle_int))
        ...
        self._dll.vm43_set_star_detector(ctypes.c_void_p(sdet_handle_int))
    ...
    def solve(self,
              ...
              pixel_size_um: float,
              params: Optional[VM43SolveParams] = None,
              log_dir: Optional[str] = None) -> Dict[str, Any]:
        """一键求解 (与 V4.1/V4.2 接口兼容)
        ...
        Args:
            ...
            log_dir: 日志目录 (None 用默认 logs/v4_3/<frame>/)
        ...
        """
        if self._closed:
            raise RuntimeError("V43Solver 已关闭, 不能再调用")
        ...
        if log_dir is None:
            log_dir = os.path.join(
                _PROJECT_ROOT, "lib", "plate_solve", "logs", "v4_3", frame_base)
        os.makedirs(log_dir, exist_ok=True)

        # --- 参数准备 ---
        if params is None:
            params = VM43SolveParams()
            self._dll.vm43_get_default_params(ctypes.byref(params))
        ...
        result = _VM43SolveResult()

        logger.info("[%s] vm43_solve 开始 (RA=%.6f Dec=%.6f focal=%.1fmm pixel=%.2fum)",
                    frame_base, ra, dec, focal_length_mm, pixel_size_um)

        ret = self._dll.vm43_solve(
            image_path_bytes,
        ...
        # --- 释放 C++ 内部分配的 cu/cw ---
        self._dll.vm43_free_result(ctypes.byref(result))

        if ret != 0 or not result.success:
            logger.error("[%s] vm43_solve 失败: %s", frame_base,
                         solve_result.get("error", "未知错误"))
        else:
            logger.info("[%s] vm43_solve 成功: RMS=%.4fpx S_robust=%.4f\" matched=%d iters=%d",
    ...
    @staticmethod
    def _convert_result(result: _VM43SolveResult,
                        frame_base: str,
                        log_dir: str) -> Dict[str, Any]:
        """将 C 结构体 _VM43SolveResult 转换为 Python dict (V4.1/V4.2 兼容)"""
