"""
统一IO接口封装层 (Task 1)
功能: 封装现有star_detector/gaia_client/astro_image_io/vector_match_v2的Python绑定，提供统一接口
用途: ADV-PA盲解析管线通过此层调用星点检测、Gaia DR3查询、图像读取、s0读取，避免重复开发
依赖:
    - lib.star_detector.python.star_detector (StarDetector, SDetParamsPy)
    - lib.plate_solve.python.vector_match_v2 (GaiaClientPy, gnomonic_forward, gnomonic_inverse, _umeyama)
...
from lib.plate_solve.python.vector_match_v2 import GaiaClientPy, gnomonic_forward, gnomonic_inverse, _umeyama
...
def get_s0_from_header(metadata: ImageMetadataPy) -> Optional[float]:
    """
